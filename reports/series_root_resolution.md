# Series root resolution

- Missing-series packets inspected: 1129
- Packets with at least one root candidate: 1125
- Packets with a single resolved provisional root: 1123

| GSC | Root candidates | Resolved root | Source |
| --- | --- | --- | --- |
| `01-09` | 豦→ɡa [*ɡa] (None); 豦→ra [*rˤa {*[r]ˤa}] (None); 豦→ka [*kas] (None) | `ra` | `head_graph_supported_root` |
| `01-20` | 䀠→kuak [*kʷak-s {*[k]ʷ(r)ak-s}] (0096a) | `kuak` | `head_graph_oc_bs` |
| `01-40` | 樗→kua [*kʷʰla̠] (None); 樗→ra [*rˤa {*[r]ˤa}] (None) | `kua` | `head_graph_shengfu_head` |
| `01-41` | 杵→qa [*t.qʰaʔ] (0086a) | `qa` | `head_graph_oc_bs` |
| `01-46` | 兔→la [*l̥ˤa-s] (0063a) | `la` | `head_graph_oc_bs` |
| `01-47` | 邪→qa [*ɢa {*[ɢ](r)A}] (0047a); 邪→la [*sə.la] (0047a) | `qa` | `head_graph_supported_root` |
| `01-51` | 𧆨→ra [*rˤa {*[r]ˤa}] (0069a) | `ra` | `head_graph_oc_bs` |
| `02-01` | 各→kak [*kˤak] (0766a) | `kak` | `head_graph_oc_bs` |
| `02-02` | 𧮫→ɡak [*ɡaɡ] (None) | `ɡak` | `head_graph_oc_shengfu` |
| `02-03` | 𢧢→kak [*krak {*[k]rak}] (None) | `kak` | `same_character_series_root` |
| `02-04` | 丮→kak [*kʳaɡ] (None); 丮→ɡak [*ɡaɡ] (None) | `kak` | `head_graph_shengfu_head` |
| `02-05` | 𡭴→kak [*kʰʳaɡ] (None) | `kak` | `head_graph_oc_shengfu` |
| `02-06` | 郭→kuak [*kʷˤak] (0774a) | `kuak` | `head_graph_oc_bs` |
| `02-08` | 蒦→quak [*m-qʷˤrak] (0784a) | `quak` | `head_graph_oc_bs` |
| `02-09` | 壑→qak [*qʰˤak] (0767a) | `qak` | `head_graph_oc_bs` |
| `02-10` | 赫→qak [*qʰˤrak] (0779a) | `qak` | `head_graph_oc_bs` |
| `02-11` | 覤→kak [*kʰʳaɡ] (None); 覤→ra [*rˤa {*[r]ˤa}] (None); 覤→qyk [*qʰɯɡ] (None) | `kak` | `head_graph_supported_root` |
| `02-12` | 霍→quok [*qʷʰo̠ɡʷ] (None) | `quok` | `head_graph_oc_shengfu` |
| `02-13` | 亞→ʔak [*ʔˤrak-s] (0805a) | `ʔak` | `head_graph_oc_bs` |
| `02-14` | 屰→ṅak [*ŋrak] (0788a) | `ṅak` | `head_graph_oc_bs` |
| `02-15` | 噩→ṅak [*ŋa̠ɡ] (None) | `ṅak` | `head_graph_oc_shengfu` |
| `02-16` | 度→tak [*dˤak {*[d]ˤak}] (0801a) | `tak` | `head_graph_oc_bs` |
| `02-17` | 石→tak [*dak {*dAk}] (0795a) | `tak` | `head_graph_oc_bs` |
| `02-18` | 庶→tak [*s-tak-s] (0804a) | `tak` | `head_graph_oc_bs` |
| `02-19` | 炙→tak [*tak-s {*tAk-s}] (0791a) | `tak` | `head_graph_oc_bs` |
| `02-20` | 尺→tak [*tʰak {*tʰAk}] (0794a) | `tak` | `head_graph_oc_bs` |
| `02-21` | 辵→tak [*tʰaɡ] (None) | `tak` | `head_graph_oc_shengfu` |
| `02-22` | 乇→tak [*tˤrak] (0780a) | `tak` | `head_graph_oc_bs` |
| `02-23` | 㡿→qak [*t-qʰak {*t-qʰAk}] (0792a) | `qak` | `head_graph_oc_bs` |
| `02-24` | 赤→qak [*t-qʰak {*[t-qʰ](r)Ak}] (0793a) | `qak` | `head_graph_oc_bs` |
| `02-25` | 睪→lak [*ɡ‧laɡ] (None) | `lak` | `head_graph_oc_shengfu` |
| `02-26` | 射→lak [*Cə.lak-s {*Cə.lAk-s}] (0807a) | `lak` | `head_graph_oc_bs` |
| `02-27` | 亦→qak [*ɢak {*ɢ(r)Ak}] (0800a) | `qak` | `head_graph_oc_bs` |
| `02-28` | 夕→rak [*s-N-rak {*s-N-rAk}] (0796a) | `rak` | `head_graph_oc_bs` |
| `02-29` | 席→tak [*s-dak {*s-[d]Ak}] (0797a) | `tak` | `head_graph_oc_bs` |
| `02-31` | 乍→tsak [*dzˤrak-s {*[dz]ˤrak-s}] (0806a) | `tsak` | `head_graph_oc_bs` |
| `02-32` | 昔→sak [*sak {*[s]Ak}] (0798a) | `sak` | `head_graph_oc_bs` |
| `02-33` | 索→sak [*sˤak {*[s]ˤak}] (0770a) | `sak` | `head_graph_oc_bs` |
| `02-34` | 朔→ṅak [*s.ŋrak] (0769a) | `ṅak` | `head_graph_oc_bs` |
| `02-35` | 舄→qak [*s.qʰak {*s.qʰAk}] (0799a) | `qak` | `head_graph_oc_bs` |
| `02-36` | 亳→pak [*bda̠ɡ] (None) | `pak` | `head_graph_oc_shengfu` |
| `02-37` | 百→pak [*pˤrak] (0781a) | `pak` | `head_graph_oc_bs` |
| `02-38` | 白→pak [*bˤrak] (0782a) | `pak` | `head_graph_oc_bs` |
| `02-39` | 霸→pak [*pˤrak-s] (0772b) | `pak` | `head_graph_oc_bs` |
| `02-40` | 莫→mak [*mˤak] (0802a) | `mak` | `head_graph_oc_bs` |
| `03-01` | 亢→ṅaṅ [*k.ŋˤaŋ] (0698a) | `ṅaṅ` | `head_graph_oc_bs` |
| `03-02` | 岡→kaṅ [*kˡa̠ŋ] (None) | `kaṅ` | `head_graph_oc_shengfu` |
| `03-03` | 畺→kaṅ [*kaŋ] (0710a) | `kaṅ` | `head_graph_oc_bs` |
| `03-04` | 强→kaṅ [*m-kaŋʔ] (0713a) | `kaṅ` | `head_graph_oc_bs` |
| `03-05` | 姜→qaṅ [*C.qaŋ] (0711a) | `qaṅ` | `head_graph_oc_bs` |
| `03-06` | 羌→qaṅ [*C.qʰaŋ] (0712a) | `qaṅ` | `head_graph_oc_bs` |
| `03-07` | 竟→kaṅ [*kʳaŋs] (None) | `kaṅ` | `head_graph_oc_shengfu` |
| `03-08` | 慶→kaṅ [*kʰʳaŋs] (None) | `kaṅ` | `head_graph_oc_shengfu` |
| `03-09` | 競→kaṅ [*m-kraŋʔ-s ~ C-kraŋʔ-s] (0754a) | `kaṅ` | `head_graph_oc_bs` |
| `03-10` | 京→kaṅ [*kraŋ {*[k]raŋ}] (0755a) | `kaṅ` | `head_graph_oc_bs` |
| `03-11` | 更→kaṅ [*kˤraŋ] (0745a) | `kaṅ` | `head_graph_oc_bs` |
| `03-12` | 庚→kaṅ [*kˤraŋ] (0746a); 唐→raṅ [*N-rˤaŋ {*[N-]rˤaŋ}] (0700a) | `kaṅ` | `merged_packet_primary_head` |
| `03-13` | 羹→kaṅ [*kˤraŋ] (0747a) | `kaṅ` | `head_graph_oc_bs` |
| `03-14` | 行→kaṅ [*Cə.gˤraŋ] (0748a) | `kaṅ` | `head_graph_oc_bs` |
| `03-15` | 杏→kaṅ [*C.gˤraŋʔ {*C.[g]ˤraŋʔ}] (0749a) | `kaṅ` | `head_graph_oc_bs` |
| `03-16` | 皀→qaṅ [*qʰaŋ] (0714a) | `qaṅ` | `head_graph_oc_bs` |
| `03-17` | 享→qaṅ [*qʰˤraŋ {*[qʰ]ˤraŋ} (W dial.)] (0716a) | `qaṅ` | `head_graph_oc_bs` |
| `03-18` | 向→qaṅ [*qʰaŋs] (None) | `qaṅ` | `head_graph_oc_shengfu` |
| `03-19` | 香→qaṅ [*qʰaŋ] (0717a) | `qaṅ` | `head_graph_oc_bs` |
| `03-20` | 央→ʔaṅ [*ʔaŋ] (0718a) | `ʔaṅ` | `head_graph_oc_bs` |
| `03-21` | 影→qaṅ [*qraŋʔ] (0756a) | `qaṅ` | `head_graph_oc_bs` |
| `03-22` | 光→kuaṅ [*kʷˤaŋ] (0706a) | `kuaṅ` | `head_graph_oc_bs` |
| `03-23` | 黃→kuaṅ [*N-kʷˤaŋ] (0707a) | `kuaṅ` | `head_graph_oc_bs` |
| `03-24` | 皇→quaṅ [*ɢʷˤaŋ {*[ɢ]ʷˤaŋ}] (0708a) | `quaṅ` | `head_graph_oc_bs` |
| `03-25` | 冏→kuaṅ [*kʷʳaŋʔ] (None) | `kuaṅ` | `same_character_series_root` |
| `03-26` | 王→quaṅ [*ɢʷaŋ] (0739a) | `quaṅ` | `head_graph_oc_bs` |
| `03-27` | 永→quaṅ [*ɢʷraŋʔ {*[ɢ]ʷraŋʔ}] (0764a) | `quaṅ` | `head_graph_oc_bs` |
| `03-28` | 兄→maṅ [*m̥raŋ] (0765a) | `maṅ` | `head_graph_oc_bs` |
| `03-29` | 卬→ṅaṅ [*ŋˤaŋ] (0699a) | `ṅaṅ` | `head_graph_oc_bs` |
| `03-30` | 宕→ɡlaṅ [*ɡla̠ŋs] (None); 宕→tak [*s-dak {*s-[d]Ak}] (None) | `ɡlaṅ` | `head_graph_mc_coda` |
| `03-31` | 章→taṅ [*taŋ] (0723a) | `taṅ` | `head_graph_oc_bs` |
| `03-32` | 尚→taṅ [*daŋ-s {*[d]aŋ-s}] (0725a) | `taṅ` | `head_graph_oc_bs` |
| `03-33` | 上→taṅ [*daŋʔ-s {*[d]aŋʔ-s}] (0726a) | `taṅ` | `head_graph_oc_bs` |
| `03-34` | 昌→kaṅ [*kʰʲaŋ] (None) | `kaṅ` | `head_graph_oc_shengfu` |
| `03-35` | 長→yṅ [*Cə-N-traŋ {*Cə-[N]-traŋ}] (0721a); 長→taṅ [*traŋʔ] (0721a) | `taṅ` | `packet_bs_majority` |
| `03-36` | 丈→taṅ [*draŋʔ {*[d]raŋʔ}] (0722a) | `taṅ` | `head_graph_oc_bs` |
| `03-37` | 鬯→taṅ [*tʰraŋ-s] (0719a) | `taṅ` | `head_graph_oc_bs` |
| `03-38` | 昜→laṅ [*laŋ] (0720a) | `laṅ` | `head_graph_oc_bs` |
| `03-39` | 羊→qaṅ [*ɢaŋ] (0732a) | `qaṅ` | `head_graph_oc_bs` |
| `03-40` | 商→taṅ [*s-taŋ] (0734a) | `taṅ` | `head_graph_oc_bs` |
| `03-41` | 象→taṅ [*s-daŋʔ {*s-[d]aŋʔ}] (0728a) | `taṅ` | `head_graph_oc_bs` |
| `03-42` | 襄→naṅ [*s-naŋ] (0730a) | `naṅ` | `head_graph_oc_bs` |
| `03-43` | 良→raṅ [*raŋ {*[r]aŋ}] (0735a) | `raṅ` | `head_graph_oc_bs` |
| `03-44` | 兩→raṅ [*p.raŋʔ] (0736a) | `raṅ` | `head_graph_oc_bs` |
| `03-45` | 量→raṅ [*raŋ {*[r]aŋ}] (0737a) | `raṅ` | `head_graph_oc_bs` |
| `03-46` | 梁→raṅ [*raŋ {*[r]aŋ}] (0738a) | `raṅ` | `head_graph_oc_bs` |
| `03-47` | 葬→tsaṅ [*tsˤaŋ-s {*[ts]ˤaŋ-s}] (0702a) | `tsaṅ` | `head_graph_oc_bs` |
| `03-48` | 倉→raṅ [*[s.r̥]ˤaŋ] (0703a); 倉→tsaṅ [*tsʰˤaŋ {*[tsʰ]ˤaŋ}] (0703a) | `tsaṅ` | `head_graph_mc_onset` |
| `03-49` | 爿→saṅ [*skʰaŋ] (None) | `saṅ` | `same_character_series_root` |
| `03-50` | 廧→saṅ [*skʰaŋ] (None) | `saṅ` | `same_character_series_root` |
| `03-52` | 匠→paṅ [*S.baŋ-s {*S.[b]aŋ-s}] (0729a) | `paṅ` | `head_graph_oc_bs` |
| `03-54` | 喪→maṅ [*s-mˤaŋ {*[s-m]ˤaŋ}] (0705a) | `maṅ` | `head_graph_oc_bs` |
| `03-55` | 相→saṅ [*saŋ {*[s]aŋ}] (0731a) | `saṅ` | `head_graph_oc_bs` |
| `03-56` | 𡙁→saṅ [*sraŋʔ] (None) | `saṅ` | `same_character_series_root` |
| `03-57` | 方→paṅ [*C.paŋ] (0740a) | `paṅ` | `head_graph_oc_bs` |
| `03-58` | 匚→paṅ [*paŋ] (0741a) | `paṅ` | `head_graph_oc_bs` |
| `03-59` | 彭→paṅ [*C.bˤraŋ {*C.[b]ˤraŋ}] (0750a) | `paṅ` | `head_graph_oc_bs` |
| `03-60` | 烹→qaṅ [*p.qʰˤraŋ {*[p.qʰ]ˤraŋ}] (0751a) | `qaṅ` | `head_graph_oc_bs` |
| `03-61` | 丙→paṅ [*praŋʔ] (0757a) | `paṅ` | `head_graph_oc_bs` |
| `03-62` | 秉→paṅ [*praŋʔ] (0758a) | `paṅ` | `head_graph_oc_bs` |
| `03-63` | 兵→paṅ [*praŋ] (0759a) | `paṅ` | `head_graph_oc_bs` |
| `03-64` | 莽→maṅ [*ma̠ŋʔ] (None); 莽→ma [*ma̠ʔ] (None) | `maṅ` | `head_graph_mc_coda` |
| `03-65` | 亡→maṅ [*maŋ] (0742a) | `maṅ` | `head_graph_oc_bs` |
| `03-66` | 𦣠→maṅ [*maŋ] (None) | `maṅ` | `same_character_series_root` |
| `03-67` | 网→maṅ [*maŋʔ] (0744a) | `maṅ` | `head_graph_oc_bs` |
| `03-68` | 明→maṅ [*mraŋ] (0760a) | `maṅ` | `head_graph_oc_bs` |
| `03-69` | 皿→maṅ [*maŋʔ] (None) | `maṅ` | `head_graph_oc_shengfu` |
| `04-01` | 改→qy [*C.qˤəʔ] (0936a) | `qy` | `head_graph_oc_bs` |
| `04-02` | 亥→ky [*gˤəʔ {*[g]ˤəʔ}] (0937a) | `ky` | `head_graph_oc_bs` |
| `04-03` | 戒→kyk [*kˤrək-s ~ kˤrək] (0990a) | `kyk` | `head_graph_oc_bs` |
| `04-04` | 其→ky [*gə] (0952a) | `ky` | `head_graph_oc_bs` |
| `04-05` | 己→ky [*kəʔ {*k(r)əʔ}] (0953a) | `ky` | `head_graph_oc_bs` |
| `04-06` | 龜→kuy [*kʷə {*[k]ʷə}] (0985a) | `kuy` | `head_graph_oc_bs` |
| `04-07` | 簋→kuu [*kʷruʔ] (0986a) | `kuu` | `head_graph_oc_bs` |
| `04-08` | 𣪘→kuu [*kʷruʔ] (None) | `kuu` | `same_character_series_root` |
| `04-09` | 頯→kuy [*gʷrə {*[g]ʷrə}] (0988a) | `kuy` | `head_graph_oc_bs` |
| `04-10` | 逵→ɡy [*ɡʷʳɯ] (None); 逵→ruk [*k.ruk] (None) | `ɡy` | `head_graph_shengfu_head` |
| `04-11` | 怪→kut [*kʳu̠ds] (None) | `kut` | `head_graph_oc_shengfu` |
| `04-12` | 九→ku [*kuʔ {*[k]uʔ}] (0992a) | `ku` | `head_graph_oc_bs` |
| `04-13` | 久→kuy [*kʷəʔ {*[k]ʷəʔ}] (0993a) | `kuy` | `head_graph_oc_bs` |
| `04-14` | 丘→kuy [*kʷʰə {*[k]ʷʰə}] (0994a) | `kuy` | `head_graph_oc_bs` |
| `04-15` | 求→ku [*gu {*[g](r)u}] (1066a) | `ku` | `head_graph_oc_bs` |
| `04-16` | 臼→kuy [*C.gʷəʔ {*C.[g]ʷ(r)əʔ}] (1067a) | `kuy` | `head_graph_oc_bs` |
| `04-17` | 又→quy [*ɢʷəʔ-s {*[ɢ]ʷəʔ-s}] (0995a) | `quy` | `head_graph_oc_bs` |
| `04-18` | 尤→quy [*ɢʷə {*[ɢ]ʷə}] (0996a) | `quy` | `head_graph_oc_bs` |
| `04-19` | 郵→quy [*ɢʷɯ] (None) | `quy` | `head_graph_oc_shengfu` |
| `04-20` | 灰→quy [*qʷʰɯ̠] (None) | `quy` | `head_graph_oc_shengfu` |
| `04-21` | 喜→qy [*qʰəʔ {*qʰ(r)əʔ}] (0955a) | `qy` | `head_graph_oc_bs` |
| `04-22` | 醫→qy [*qɯ] (None); 醫→qi [*qi̠s] (None) | `qy` | `head_graph_shengfu_head` |
| `04-23` | 疑→ṅy [*ŋə {*[ŋ](r)ə} (< uvular)] (0956a) | `ṅy` | `head_graph_oc_bs` |
| `04-24` | 牛→ṅy [*ŋʷə {*[ŋ]ʷə} (< uvular?)] (0998a) | `ṅy` | `head_graph_oc_bs` |
| `04-25` | 臺→ly [*lˤə] (0939a) | `ly` | `head_graph_oc_bs` |
| `04-26` | 止→ty [*təʔ] (0961a) | `ty` | `head_graph_oc_bs` |
| `04-27` | 之→ty [*tə] (0962a) | `ty` | `head_graph_oc_bs` |
| `04-28` | 市→ty [*C.dəʔ {*C.[d]əʔ}] (0963a) | `ty` | `head_graph_oc_bs` |
| `04-29` | 止→ty [*təʔ] (0961a) | `ty` | `head_graph_oc_bs` |
| `04-30` | 㠯→ly [*ləʔ] (0976a) | `ly` | `head_graph_oc_bs` |
| `04-31` | 已→qy [*ɢəʔ] (0977a) | `qy` | `head_graph_oc_bs` |
| `04-32` | 巳→qy [*s-ɢəʔ {*s-[ɢ]əʔ}] (0967a) | `qy` | `head_graph_oc_bs` |
| `04-33` | 辭→ly [*sə.lə] (0968a) | `ly` | `head_graph_oc_bs` |
| `04-34` | 𦣞→qy [*ɢə {*[ɢ](r)ə}] (0960a) | `qy` | `head_graph_oc_bs` |
| `04-35` | 里→ry [*rəʔ {*(mə.)rəʔ}] (0978a) | `ry` | `head_graph_oc_bs` |
| `04-36` | 𠭰→ry [*ɡ‧rɯ] (None); 氂→myw [*mɯ̠w] (None); 氂→maw [*C.mˤaw] (None); 犛→my [*mɡʳɯ̠] (None); 斄→saṅ [*skʰaŋ] (None) | `ry` | `head_graph_supported_root` |
| `04-37` | 李→ry [*C.rəʔ] (0980a) | `ry` | `head_graph_oc_bs` |
| `04-38` | 乃→ny [*nˤəʔ {*nˤə(ŋ)ʔ}] (0945a) | `ny` | `head_graph_oc_bs` |
| `04-39` | 迺→ny [*nˤəʔ] (None) | `ny` | `same_character_series_root` |
| `04-40` | 耳→ny [*C.nəʔ] (0981a) | `ny` | `head_graph_oc_bs` |
| `04-41` | 而→ny [*nə] (0982a) | `ny` | `head_graph_oc_bs` |
| `04-42` | 災→tsy [*tsˤə {*[ts]ˤə}] (0940a) | `tsy` | `head_graph_oc_bs` |
| `04-43` | 再→tsy [*tsˤə-s {*[ts]ˤə(ʔ)-s}] (0941a) | `tsy` | `head_graph_oc_bs` |
| `04-44` | 彩→sy [*N-sˤrəʔ] (0942a) | `sy` | `head_graph_oc_bs` |
| `04-45` | 才→tsy [*dzˤə {*(Cə.)[dz]ˤə}] (0943a) | `tsy` | `head_graph_oc_bs` |
| `04-47` | 子→tsy [*tsəʔ {*[ts]əʔ}] (0964a) | `tsy` | `head_graph_oc_bs` |
| `04-48` | 梓→tsy [*tsəʔ {*[ts]əʔ}] (0965a) | `tsy` | `head_graph_oc_bs` |
| `04-49` | 玆→ɡin [*ɡʷi̠n] (None); 玆→quin [*ɢʷˤin {*[ɢ]ʷˤi[n]}] (None) | `ɡin` | `head_graph_shengfu_head` |
| `04-50` | 甾→sy [*sɯ̠] (None) | `sy` | `head_graph_oc_shengfu` |
| `04-51` | 士→ry [*m-s-rəʔ] (0970a) | `ry` | `head_graph_oc_bs` |
| `04-52` | 史→ry [*s-rəʔ] (0975a) | `ry` | `head_graph_oc_bs` |
| `04-53` | 司→ly [*s-lə] (0972a) | `ly` | `head_graph_oc_bs` |
| `04-54` | 思→sy [*sə {*[s]ə}] (0973a) | `sy` | `head_graph_oc_bs` |
| `04-55` | 絲→sy [*sə {*[s]ə}] (0974a) | `sy` | `head_graph_oc_bs` |
| `04-56` | 佩→pyk [*bˤək-s {*[b]ˤək-s}] (0951a) | `pyk` | `head_graph_oc_bs` |
| `04-57` | 啚→py [*pʳɯʔ] (None) | `py` | `head_graph_oc_shengfu` |
| `04-58` |  | `` | `` |
| `04-59` | 奰→pit [*bʳids] (None) | `pit` | `head_graph_oc_shengfu` |
| `04-61` | 不→py [*pə] (0999a) | `py` | `head_graph_oc_bs` |
| `04-62` | 負→py [*bəʔ {*[b]əʔ}] (1000a) | `py` | `head_graph_oc_bs` |
| `04-63` | 婦→py [*mə.bəʔ {*[m]ə.[b]əʔ} (or *tsə.-? see Min)] (1001a) | `py` | `head_graph_oc_bs` |
| `04-65` | 某→my [*məʔ] (0948a) | `my` | `head_graph_oc_bs` |
| `04-66` | 畝→my [*məʔ (< *mˤoʔ?)] (0949a) | `my` | `head_graph_oc_bs` |
| `05-01` | 克→kyk [*kʰˤək] (0903a) | `kyk` | `head_graph_oc_bs` |
| `05-02` | 革→kyk [*kˤrək] (0931a) | `kyk` | `head_graph_oc_bs` |
| `05-03` | 覈→kyk [*gˤrək {*[g]ˤr[ə]k}] (1260a) | `kyk` | `head_graph_oc_bs` |
| `05-04` | 亟→kyk [*kʰək-s {*kʰ(r)ək-s}] (0910a) | `kyk` | `head_graph_oc_bs` |
| `05-05` | 棘→kyk [*krək] (0911a) | `kyk` | `head_graph_oc_bs` |
| `05-06` | 或→quyk [*ɢʷrək {*[ɢ]ʷrək}] (0929a) | `quyk` | `head_graph_oc_bs` |
| `05-07` | 洫→mik [*m̥ik {*m̥(r)ik}] (0930a) | `mik` | `head_graph_oc_bs` |
| `05-08` | 衋→qyk [*qʰʳɯɡ] (None) | `qyk` | `head_graph_oc_shengfu` |
| `05-09` | 抑→qik [*qik] (0915a) | `qik` | `head_graph_oc_bs` |
| `05-10` | 意→ʔyk [*ʔək-s {*ʔ(r)ək-s}] (0957a) | `ʔyk` | `head_graph_oc_bs` |
| `05-11` | 得→tyk [*tˤək] (0905d) | `tyk` | `head_graph_oc_bs` |
| `05-12` | 直→tyk [*N-t<r>ək] (0919a) | `tyk` | `head_graph_oc_bs` |
| `05-13` | 戠→tyk [*tək] (0920a) | `tyk` | `head_graph_oc_bs` |
| `05-14` | 陟→tyk [*trək] (0916a) | `tyk` | `head_graph_oc_bs` |
| `05-15` | 敕→tyk [*tʰɯɡ] (None) | `tyk` | `head_graph_oc_shengfu` |
| `05-16` | 弋→lyk [*lək] (0918a) | `lyk` | `head_graph_oc_bs` |
| `05-17` | 異→qyk [*ɢək-s {*[ɢ]ək-s}] (0954a) | `qyk` | `head_graph_oc_bs` |
| `05-18` | 翊→ryk [*N.rək {*[N].rəp} (dial. > *-k)] (0912a) | `ryk` | `head_graph_oc_bs` |
| `05-19` | 食→lyk [*s-m-lək-s] (0921a); 食→myk [*mə-lək] (0921a) | `lyk` | `head_graph_supported_root` |
| `05-20` | 奭→qak [*qʰak {*[qʰ](r)Ak}] (0913a); 奭→qyk [*qʰək {*[qʰ](r)ək}] (0913a) | `qak` | `head_graph_supported_root` |
| `05-21` | 力→ryk [*k.rək] (0928a) | `ryk` | `head_graph_oc_bs` |
| `05-23` | 賊→tsyk [*k.dzˤək] (0907a) | `tsyk` | `head_graph_oc_bs` |
| `05-24` | 則→tsyk [*tsˤək {*[ts]ˤək}] (0906a) | `tsyk` | `head_graph_oc_bs` |
| `05-25` | 畟→syk [*skʰrɯɡ] (None) | `syk` | `head_graph_oc_shengfu` |
| `05-26` | 即→tsik [*tsik {*[ts]ik}] (0923a) | `tsik` | `head_graph_oc_bs` |
| `05-27` | 夨→tsyk [*tsrək {*[ts]rək}] (0924a) | `tsyk` | `head_graph_oc_bs` |
| `05-28` | 塞→syk [*sˤək-s {*[s]ˤək-s}] (0908a) | `syk` | `head_graph_oc_bs` |
| `05-29` | 息→syk [*sək] (0925a) | `syk` | `head_graph_oc_bs` |
| `05-30` | 嗇→syk [*srək] (0926a) | `syk` | `head_graph_oc_bs` |
| `05-31` | 色→syk [*srək] (0927a) | `syk` | `head_graph_oc_bs` |
| `05-32` | 北→pyk [*pˤək] (0909a) | `pyk` | `head_graph_oc_bs` |
| `05-33` | 畐→pyk [*bək {*[b]ək}] (0933a) | `pyk` | `head_graph_oc_bs` |
| `05-34` | 備→pyk [*brək-s {*[b]rək-s}] (0984d) | `pyk` | `head_graph_oc_bs` |
| `05-35` | 𠬝→pyk [*bək {*[b]ək}] (0934a) | `pyk` | `head_graph_oc_bs` |
| `05-36` | 伏→pyk [*bək-s {*[b]ək-s} (< *[b]uk-s ?)] (0935a) | `pyk` | `head_graph_oc_bs` |
| `05-37` | 冒→muk [*mˤuk-s] (1062a) | `muk` | `head_graph_oc_bs` |
| `05-38` | 黑→myk [*m̥ʰɯ̠ɡ] (None) | `myk` | `head_graph_oc_shengfu` |
| `05-39` | 牧→myk [*mək] (1037a) | `myk` | `head_graph_oc_bs` |
| `06-01` | 亙→kyṅ [*kɯ̠ŋs] (None) | `kyṅ` | `head_graph_oc_shengfu` |
| `06-02` | 肎→kyṅ [*kʰɯ̠ŋʔ] (None) | `kyṅ` | `same_character_series_root` |
| `06-03` | 兢→kyṅ [*kəŋ {*k(r)əŋ}] (0888a) | `kyṅ` | `head_graph_oc_bs` |
| `06-04` | 弓→kuyṅ [*kʷəŋ] (0901a) | `kuyṅ` | `head_graph_oc_bs` |
| `06-05` | 𠃋→kuyṅ [*kʷˤəŋ {*[k]ʷˤəŋ}] (0887a) | `kuyṅ` | `head_graph_oc_bs` |
| `06-06` | 興→qyṅ [*qʰəŋ {*qʰ(r)əŋ}] (0889a) | `qyṅ` | `head_graph_oc_bs` |
| `06-09` | 豋→tyṅ [*tˤəŋ] (0883a) | `tyṅ` | `head_graph_oc_bs` |
| `06-10` | 氶→kyṅ [*kʲɯŋʔ] (None) | `kyṅ` | `head_graph_oc_shengfu` |
| `06-11` | 徵→tyṅ [*trəŋ] (0891a) | `tyṅ` | `head_graph_oc_bs` |
| `06-12` | 爯→tyṅ [*tʰʲɯŋ] (None) | `tyṅ` | `head_graph_oc_shengfu` |
| `06-13` | 㑞→lum [*ɡ‧lums] (None); 㑞→ɡlum [*ɡlumʔ] (None); 朕→lym [*lrəmʔ] (0893f); 媵→qum [*ɢlʲums] (None); 塍→lyṅ [*Cə.ləŋ] (0893n); 螣→ɡlup [*ɡlu̠b] (None) | `ɡlum` | `head_graph_supported_root` |
| `06-14` | 孕→liṅ [*liŋ-s {*l[i]ŋ-s}] (0945j); 孕→lum [*b‧lums] (None) | `liṅ` | `head_graph_mc_coda` |
| `06-15` | 乘→lyṅ [*Cə.ləŋ] (0895a) | `lyṅ` | `head_graph_oc_bs` |
| `06-16` | 升→tyṅ [*s-təŋ] (0897a) | `tyṅ` | `head_graph_oc_bs` |
| `06-17` | 夌→ryṅ [*rəŋ {*[r]əŋ}] (0898a) | `ryṅ` | `head_graph_oc_bs` |
| `06-18` | 能→ny [*nˤə] (0885a) | `ny` | `head_graph_oc_bs` |
| `06-19` | 曾→tsyṅ [*dzˤəŋ {*[dz]ˤəŋ}] (0884a) | `tsyṅ` | `head_graph_oc_bs` |
| `06-20` | 朋→pyṅ [*bˤəŋ {*[b]ˤəŋ}] (0886a) | `pyṅ` | `head_graph_oc_bs` |
| `06-21` | 冫→  *pyṅ [*p.rəŋ (dial. > *prəŋ)] (0899a) | `  *pyṅ` | `head_graph_oc_bs` |
| `06-22` | 凭→pyṅ [*bʳɯŋ] (None) | `pyṅ` | `head_graph_oc_shengfu` |
| `06-23` | 夢→myṅ [*C.məŋ-s] (0902a) | `myṅ` | `head_graph_oc_bs` |
| `06-24` | 蠅→ryṅ [*m.rəŋ] (0892a) | `ryṅ` | `head_graph_oc_bs` |
| `07-02` | 繼→kek [*ke̠ɡs] (None) | `kek` | `head_graph_oc_shengfu` |
| `07-03` | 支→ke [*ke] (0864a) | `ke` | `head_graph_oc_bs` |
| `07-04` | 企→ke [*kʰeʔ] (None) | `ke` | `head_graph_oc_shengfu` |
| `07-05` | 只→ke [*kʲe] (None) | `ke` | `head_graph_oc_shengfu` |
| `07-06` | 氏→te [*k.deʔ] (0867a) | `te` | `head_graph_oc_bs` |
| `07-07` | 解→ke [*N.kˤreʔ {*N-kˤreʔ}] (0861a) | `ke` | `head_graph_oc_bs` |
| `07-08` | 圭→kue [*kʷˤe {*[k]ʷˤe}] (0879a) | `kue` | `head_graph_oc_bs` |
| `07-09` | 巂→so [*sqʰolʔ] (None); 巂→qu [*ɢu̠l] (None) | `ɡu` | `packet_shengfu_majority` |
| `07-10` | 規→kue [*kʷe] (0875a) | `kue` | `head_graph_oc_bs` |
| `07-11` | 兒→ṅe [*ŋe] (0873a) | `ṅe` | `head_graph_oc_bs` |
| `07-12` | 帝→tek [*tˤek-s] (0877a) | `tek` | `head_graph_oc_bs` |
| `07-13` | 知→te [*tre] (0863a) | `te` | `head_graph_oc_bs` |
| `07-14` | 是→te [*deʔ {*[d]eʔ}] (0866a) | `te` | `head_graph_oc_bs` |
| `07-15` | 卮→te [*tje] (None) | `te` | `same_character_series_root` |
| `07-16` | 豸→te [*dreʔ {*[d]reʔ}] (1238b); 豸→ɡle [*ɡleʔ] (None) | `te` | `head_graph_bs_head` |
| `07-18` | 咶→ɡot [*ɡʳo̠d] (None); 咶→kot [*kod] (None); 咶→qot [*qʰʳo̠ds] (None) | `kot` | `head_graph_supported_root` |
| `07-19` | 豕→lay [*l̥ajʔ] (1238f); 豕→le [*l̥elʔ] (None) | `lay` | `head_graph_bs_head` |
| `07-20` | 爾→ne [*nˤeʔ {*nˤe(j)ʔ}] (0359a) | `ne` | `head_graph_oc_bs` |
| `07-21` | 麗→re [*rˤe-s {*[r]ˤe-s}] (0878a) | `re` | `head_graph_oc_bs` |
| `07-22` | 蠡→re [*k‧rel] (None); 蠡→lon [*l̥ˤon-s {*l̥ˤo[n]-s}] (None); 蠡→ro [*ɡ‧ro̠l] (None) | `lon` | `head_graph_supported_root` |
| `07-23` | 荔→re [*ɡ‧rels] (None) | `re` | `head_graph_oc_shengfu` |
| `07-24` | 詈→ray [*C.raj-s] (0872a) | `ray` | `head_graph_oc_bs` |
| `07-25` | 此→tse [*tsʰeʔ {*[tsʰ]e(j)ʔ}] (0358a) | `tse` | `head_graph_oc_bs` |
| `07-26` | 斯→se [*se {*[s]e}] (0869a) | `se` | `head_graph_oc_bs` |
| `07-27` | 虒→le [*s.le] (0870a) | `le` | `head_graph_oc_bs` |
| `07-28` | 徙→say [*sajʔ {*[s]ajʔ}] (0871a) | `say` | `head_graph_oc_bs` |
| `07-29` | 卑→pe [*pe] (0874a) | `pe` | `head_graph_oc_bs` |
| `07-31` | 弭→me [*melʔ] (None); 弭→ny [*C.nəʔ] (None) | `me` | `head_graph_shengfu_head` |
| `07-33` | 買→may [*mˤrajʔ] (1240c); 買→me [*mʳe̠ʔ] (None); 買→ray [*C.raj-s] (None) | `` | `` |
| `08-01` | 毄→kek [*ke̠ɡ] (None); 毄→kik [*ki̠ɡs] (None) | `kek` | `head_graph_supported_root` |
| `08-02` | 鬲→qik [*C.qˤ<r>ik] (0855a); 鬲→rek [*k.rˤek] (0855a) | `rek` | `head_graph_supported_root` |
| `08-03` | 覡→ɡek [*ɡe̠ɡ] (None); 覡→kiy [*s-gijʔ-s] (None) | `ɡek` | `head_graph_mc_coda` |
| `08-04` | 戹→qik [*qˤ<r>ik {*qˤ<r>[i]k}] (None) | `qik` | `same_character_series_root` |
| `08-05` | 益→ek [*qik (dial. > *-ek)] (0849a) | `ek` | `head_graph_oc_bs` |
| `08-07` | 砉→quek [*qʷʰʳe̠ɡ] (None); 砉→kue [*kʷˤe {*[k]ʷˤe}] (None) | `quek` | `head_graph_mc_coda` |
| `08-08` | 役→quek [*ɢʷek] (0851a) | `quek` | `head_graph_oc_bs` |
| `08-09` | 畫→kuek [*C-gʷˤrek-s] (0847a) | `kuek` | `head_graph_oc_bs` |
| `08-10` | 狄→lek [*lˤek] (0856a) | `lek` | `head_graph_oc_bs` |
| `08-11` | 隻→tek [*tek] (1260c) | `tek` | `head_graph_oc_bs` |
| `08-12` | 易→lek [*lek-s] (0850a) | `lek` | `head_graph_oc_bs` |
| `08-13` | 秝→rek [*k‧re̠ɡ] (None) | `rek` | `head_graph_oc_shengfu` |
| `08-14` | 朿→tsek [*tsʰek-s {*[tsʰ]ek-s}] (0868a) | `tsek` | `head_graph_oc_bs` |
| `08-15` | 册→tsek [*tsʰrek {*[tsʰ](ˤ)rek}] (0845a) | `tsek` | `head_graph_oc_bs` |
| `08-16` | 筴→sek [*sʰre̠ɡ] (None); 筴→tsek [*tsʰek-s {*[tsʰ]ek-s}] (None); 筴→kep [*kʳe̠b] (None) | `kep` | `head_graph_supported_root` |
| `08-17` | 脊→tsek [*tsek {*[ts]ek}] (0852a) | `tsek` | `head_graph_oc_bs` |
| `08-18` | 析→sek [*sˤek {*[s]ˤek}] (0857a) | `sek` | `head_graph_oc_bs` |
| `08-19` | 辟→pek [*bˤek {*[b]ˤek}] (0853a) | `pek` | `head_graph_oc_bs` |
| `08-21` | 鼏→mek [*me̠ɡ] (None) | `mek` | `head_graph_oc_shengfu` |
| `08-22` | 簚→mek [*mˤek {*mˤ[e]k}] (1260e) | `mek` | `head_graph_oc_bs` |
| `08-24` | 脈→mik [*C.mˤ<r>ik {*C.mˤ<r>[i]k}] (0848a) | `mik` | `head_graph_oc_bs` |
| `09-01` | 巠→leṅ [*k.lˤeŋ] (0831a) | `leṅ` | `head_graph_oc_bs` |
| `09-02` | 殸→qeṅ [*C.qʰˤeŋ-s {*[C.qʰ]ˤeŋ-s}] (0832a); 聲→leṅ [*l̥eŋ {*[l̥]eŋ}] (0822a) | `qeṅ` | `merged_packet_primary_head` |
| `09-03` | 敬→keṅ [*kreŋ-s {*kreŋ(ʔ)-s}] (0813a) | `keṅ` | `head_graph_oc_bs` |
| `09-04` | 耕→keṅ [*kˤ<r>eŋ] (0808a) | `keṅ` | `head_graph_oc_bs` |
| `09-05` | 幸→keṅ [*gˤreŋʔ {*[g]ˤreŋʔ}] (0810a) | `keṅ` | `head_graph_oc_bs` |
| `09-06` | 耿→keṅ [*kˤreŋʔ {*[k]ˤreŋʔ}] (0809a) | `keṅ` | `head_graph_oc_bs` |
| `09-07` | 頃→kueṅ [*kʷʰeŋʔ {*[k]ʷʰeŋʔ}] (0828a) | `kueṅ` | `head_graph_oc_bs` |
| `09-08` | 冋→ɡeṅ [*ɡʷe̠ŋʔ] (None) | `ɡeṅ` | `head_graph_oc_shengfu` |
| `09-09` | 熒→queṅ [*N-qʷˤeŋ {*[N]-qʷˤeŋ}] (0843a) | `queṅ` | `head_graph_oc_bs` |
| `09-10` | 嬰→ʔeṅ [*ʔeŋ] (0814a) | `ʔeṅ` | `head_graph_oc_bs` |
| `09-11` | 丁→teṅ [*tˤeŋ] (0833a) | `teṅ` | `head_graph_oc_bs` |
| `09-12` | 鼎→teṅ [*tˤeŋʔ] (0834a) | `teṅ` | `head_graph_oc_bs` |
| `09-13` | 奠→tiṅ [*di̠ŋs] (None) | `tiṅ` | `head_graph_oc_shengfu` |
| `09-14` | 成→teṅ [*m-deŋ {*m-[d]eŋ}] (0818a) | `teṅ` | `head_graph_oc_bs` |
| `09-15` | 盈→qeṅ [*ɢleŋ] (None) | `qeṅ` | `head_graph_oc_shengfu` |
| `09-16` | 嬴→leṅ [*ɡ‧leŋ] (None) | `leṅ` | `head_graph_oc_shengfu` |
| `09-17` | 𡈼→leṅ [*l̥ˤeŋʔ] (0835a) | `leṅ` | `head_graph_oc_bs` |
| `09-18` | 霝→reṅ [*re̠ŋ] (None) | `reṅ` | `head_graph_oc_shengfu` |
| `09-19` | 令→riṅ [*riŋ] (0823a) | `riṅ` | `head_graph_oc_bs` |
| `09-20` | 寧→neṅ [*nˤeŋ] (0837a) | `neṅ` | `head_graph_oc_bs` |
| `09-21` | 晶→tseṅ [*tseŋ] (0820a) | `tseṅ` | `head_graph_oc_bs` |
| `09-22` | 井→tseṅ [*C.tseŋʔ] (0819a) | `tseṅ` | `head_graph_oc_bs` |
| `09-23` | 爭→tseṅ [*tsˤreŋ {*[ts]ˤreŋ} ~ m-tsˤreŋ (or mə-tsˤreŋ)] (0811a) | `tseṅ` | `head_graph_oc_bs` |
| `09-24` | 觲→seṅ [*sqʷʰeŋ] (None) | `seṅ` | `head_graph_oc_shengfu` |
| `09-25` | 晴→tseṅ [*N-tsʰeŋ] (812); 晴→seṅ [*sɡleŋ] (None) | `tseṅ` | `head_graph_bs_head` |
| `09-26` | 平→peṅ [*breŋ] (0825a) | `peṅ` | `head_graph_oc_bs` |
| `09-27` | 甹→leṅ [*p.l̥ˤeŋ {*[p.l̥]ˤeŋ}] (0839a) | `leṅ` | `head_graph_oc_bs` |
| `09-28` | 竝→peṅ [*bˤeŋʔ {*[b]ˤeŋʔ}] (None) | `peṅ` | `same_character_series_root` |
| `09-29` | 并→peṅ [*peŋ] (None) | `peṅ` | `head_graph_oc_shengfu` |
| `09-30` | 冥→meṅ [*mˤeŋ] (0841a) | `meṅ` | `head_graph_oc_bs` |
| `09-31` | 名→meṅ [*C.meŋ] (0826a) | `meṅ` | `head_graph_oc_bs` |
| `09-32` | 命→  *meṅ [*mə-riŋ-s (dial. > *mreŋ-s)] (0762a) | `  *meṅ` | `head_graph_oc_bs` |
| `09-33` | 鳴→reṅ [*m.reŋ] (0827a) | `reṅ` | `head_graph_oc_bs` |
| `10-14` | 𧱓→to [*to̠s] (None); 𧱓→tok [*tʰrok] (None) | `to` | `head_graph_shengfu_head` |
| `10-15` | 斲→tok [*Cə.tˤrok] (1235b); 斲→rok [*rto̠ɡ] (None); 鬭→to [*to̠s] (None) | `` | `` |
| `10-17` | 鬥→to [*to̠s] (None) | `to` | `head_graph_oc_shengfu` |
| `10-20` | 馵→tok [*tʲoɡs] (None) | `tok` | `head_graph_oc_shengfu` |
| `10-21` | 殳→to [*do {*[d]o}] (0130a) | `to` | `head_graph_oc_bs` |
| `10-25` | 窳→lo [*loʔ] (None) | `lo` | `head_graph_oc_shengfu` |
| `10-26` | 戍→no [*n̥o-s {*[n̥]o-s}] (1243c); 戍→lo [*l̥os] (None) | `no` | `head_graph_bs_head` |
| `10-28` | 陋→ro [*rˤo-s {*[r]ˤo-s}] (1246b) | `ro` | `head_graph_oc_bs` |
| `10-30` | 須→no [*s-no] (0133a) | `no` | `head_graph_oc_bs` |
| `10-31` | 需→so [*sno] (None) | `so` | `head_graph_oc_shengfu` |
| `10-32` | 乳→no [*noʔ] (0135a) | `no` | `head_graph_oc_bs` |
| `10-34` | 奏→tso [*tsˤo-s {*tsˤo(ʔ)-s}] (1229a) | `tso` | `head_graph_oc_bs` |
| `10-35` | 取→tso [*tsʰoʔ-s] (0131a); 最→tsot [*tsˤot-s {*[ts]ˤot-s}] (0325a) | `tso` | `merged_packet_primary_head` |
| `10-36` | 芻→tso [*tsʰro {*[tsʰ]ro}] (0132a) | `tso` | `head_graph_oc_bs` |
| `10-37` | 叜→  *su [*s.ruʔ (dial. > *sruʔ > srjuwX > sruwX > suwX)] (None); 叟→su [*sɡˡu̠ʔ] (None) | `  *su` | `head_graph_supported_root` |
| `10-38` | 裒→pu [*bu̠] (None) | `pu` | `head_graph_oc_shengfu` |
| `10-39` | 付→po [*pos] (None) | `po` | `head_graph_oc_shengfu` |
| `10-40` | 鳧→po [*bo] (None) | `po` | `head_graph_oc_shengfu` |
| `11-05` | 局→kok [*gok {*[g](r)ok}] (1214a) | `kok` | `head_graph_oc_bs` |
| `11-06` | 屋→qok [*qo̠ɡ] (None) | `qok` | `head_graph_oc_shengfu` |
| `11-07` | 勖→mok [*m̥ʰoɡ] (None); 勖→muk [*mˤuk-s] (None) | `mok` | `head_graph_shengfu_head` |
| `11-08` | 岳→ṅok [*ŋˤrok {*[ŋ]ˤrok}] (1227a) | `ṅok` | `head_graph_oc_bs` |
| `11-09` | 玉→ṅok [*ŋok {*[ŋ](r)ok} (< uvular?)] (1216a) | `ṅok` | `head_graph_oc_bs` |
| `11-10` | 獄→ṅok [*ŋrok {*[ŋ]rok}] (1215a) | `ṅok` | `head_graph_oc_bs` |
| `11-11` | 禿→tok [*tʰo̠ɡ] (None) | `tok` | `head_graph_oc_shengfu` |
| `11-12` | 蜀→tok [*dok {*[d]ok}] (1224a) | `tok` | `head_graph_oc_bs` |
| `11-13` | 豖→tok [*tʰrok] (1218a) | `tok` | `head_graph_oc_bs` |
| `11-14` | 谷→qok [*C.qˤok] (1202a) | `qok` | `head_graph_oc_bs` |
| `11-15` | 彔→rok [*pə.rˤok] (1208a); 剝→pyk [*pˤrok ~ *mə-pˤrok] (1228a) | `rok` | `merged_packet_primary_head` |
| `11-16` | 鹿→myk [*mə-rˤok] (1209a) | `myk` | `head_graph_oc_bs` |
| `11-17` | 辱→nok [*nok] (1223a) | `nok` | `head_graph_oc_bs` |
| `11-18` | 族→tsok [*dzˤok {*[dz]ˤok}] (1206a) | `tsok` | `head_graph_oc_bs` |
| `11-19` | 足→tsok [*tsok {*[ts]ok}] (1219a) | `tsok` | `head_graph_oc_bs` |
| `11-20` | 粟→sok [*skˡoɡ] (None) | `sok` | `head_graph_oc_shengfu` |
| `11-21` | 束→tok [*s-tʰok] (1222a) | `tok` | `head_graph_oc_bs` |
| `11-22` | 卜→pok [*pˤok] (1210a) | `pok` | `head_graph_oc_bs` |
| `11-23` | 菐→puk [*puɡ] (None); 菐→pok [*bo̠ɡ] (None) | `pok` | `packet_bs_consensus` |
| `11-24` | 木→mok [*C.mˤok] (1212a) | `mok` | `head_graph_oc_bs` |
| `12-01` | 工→koṅ [*kˤoŋ] (1172a) | `koṅ` | `head_graph_oc_bs` |
| `12-02` | 孔→koṅ [*kʰˤoŋʔ {*[k]ʰˤoŋʔ}] (1174a) | `koṅ` | `head_graph_oc_bs` |
| `12-03` | 廾→koṅ [*koŋʔ {*k(r)oŋʔ}] (1182a) | `koṅ` | `head_graph_oc_bs` |
| `12-05` | 凶→qoṅ [*qʰoŋ {*qʰ(r)oŋ}] (1183a) | `qoṅ` | `head_graph_oc_bs` |
| `12-07` | 妐→qoṅ [*t-qoŋ] (1189a) | `qoṅ` | `head_graph_oc_bs` |
| `12-08` | 重→toṅ [*m-troŋ {*[m]-troŋ}] (1188a) | `toṅ` | `head_graph_oc_bs` |
| `12-09` | 同→loṅ [*lˤoŋ] (1176a) | `loṅ` | `head_graph_oc_bs` |
| `12-10` | 用→loṅ [*m.loŋ-s] (1185a) | `loṅ` | `head_graph_oc_bs` |
| `12-11` | 容→qoṅ [*ɢoŋ {*[ɢ](r)oŋ}] (1187a) | `qoṅ` | `head_graph_oc_bs` |
| `12-12` | 舂→toṅ [*s-toŋ] (1192a) | `toṅ` | `head_graph_oc_bs` |
| `12-13` | 公→qoṅ [*C.qˤoŋ] (1173a) | `qoṅ` | `head_graph_oc_bs` |
| `12-14` | 弄→roṅ [*rˤoŋ-s {*[r]ˤoŋ-s}] (1180a) | `roṅ` | `head_graph_oc_bs` |
| `12-15` | 龍→myṅ [*mə-roŋ {*[mə]-roŋ}] (1193a); 龍→moṅ [*mˤroŋ] (1193a) | `roṅ` | `packet_bs_majority` |
| `12-16` | 茸→noṅ [*nʲoŋ] (None) | `noṅ` | `head_graph_oc_shengfu` |
| `12-18` | 宂→noṅ [*nʲoŋʔ] (None) | `noṅ` | `head_graph_oc_shengfu` |
| `12-19` | 囱→soṅ [*skʰo̠ŋ] (None) | `soṅ` | `same_character_series_root` |
| `12-20` | 㚇→soṅ [*sko̠ŋ] (None) | `soṅ` | `head_graph_oc_shengfu` |
| `12-21` | 叢→soṅ [*sɡo̠ŋ] (None); 叢→tso [*tsʰoʔ-s] (None) | `soṅ` | `head_graph_mc_coda` |
| `12-22` | 从→soṅ [*zoŋ] (None) | `soṅ` | `head_graph_oc_shengfu` |
| `12-23` | 送→soṅ [*sˤoŋ-s {*[s]ˤoŋ-s}] (1179a) | `soṅ` | `head_graph_oc_bs` |
| `12-24` | 雙→soṅ [*sˤroŋ {*[s]ˤroŋ}] (1200a) | `soṅ` | `head_graph_oc_bs` |
| `12-25` | 丰→poṅ [*pʰoŋ] (None) | `poṅ` | `head_graph_oc_shengfu` |
| `12-26` | 尨→moṅ [*mˤroŋ] (1201a) | `moṅ` | `head_graph_oc_bs` |
| `12-27` | 蒙→moṅ [*mˤoŋ] (1181a) | `moṅ` | `head_graph_oc_bs` |
| `13-01` | 皐→ku₄ [*kˤu {*[k]ˤu}] (None) | `ku₄` | `same_character_series_root` |
| `13-02` | 杲→ku [*ku̠ʔ] (None) | `ku` | `head_graph_oc_shengfu` |
| `13-03` | 丂→ku [*kʰu̠ʔ] (None); 苛→ɡa [*ɡa̠l] (None) | `ku` | `head_graph_supported_root` |
| `13-04` | 昊→ɡu [*ɡu̠ʔ] (None) | `ɡu` | `head_graph_oc_shengfu` |
| `13-05` | 顥→ɡu [*ɡu̠ʔ] (None) | `ɡu` | `head_graph_oc_shengfu` |
| `13-06` | 韭→ku [*s.kuʔ {*s.[k](r)uʔ}] (1065a) | `ku` | `head_graph_oc_bs` |
| `13-07` | 咎→ku [*guʔ {*[g](r)uʔ}] (1068a) | `ku` | `head_graph_oc_bs` |
| `13-08` | 丩→kiw [*kriw] (1064a) | `kiw` | `head_graph_oc_bs` |
| `13-09` | 好→qu [*qʰˤuʔ-s] (1044a) | `qu` | `head_graph_oc_bs` |
| `13-10` | 薅→qu [*qʰˤu] (1244d) | `qu` | `head_graph_oc_bs` |
| `13-11` | 休→qu [*qʰu {*qʰ(r)u}] (1070a) | `qu` | `head_graph_oc_bs` |
| `13-12` | 臭→qu [*t-qʰu-s {*t-qʰu(ʔ)-s}] (1088a) | `qu` | `head_graph_oc_bs` |
| `13-13` | 醜→qu [*t.qʰuʔ] (1089a) | `qu` | `head_graph_oc_bs` |
| `13-14` | 憂→qu [*qˡu] (None) | `qu` | `head_graph_oc_shengfu` |
| `13-15` | 麀→qu [*qu] (None) | `qu` | `head_graph_oc_shengfu` |
| `13-16` | 幺→ʔiw [*ʔˤiw {*ʔˤ[i]w}] (1115a) | `ʔiw` | `head_graph_oc_bs` |
| `13-17` | 窅→qew [*qʳe̠w] (None) | `qew` | `head_graph_oc_shengfu` |
| `13-18` | 饕→ku [*kʰlu̠] (None) | `ku` | `head_graph_oc_shengfu` |
| `13-19` | 舟→tu [*tu] (1084a) | `tu` | `head_graph_oc_bs` |
| `13-20` | 州→tu [*tu] (1086a) | `tu` | `head_graph_oc_bs` |
| `13-21` | 讎→tu [*du {*[d]u}] (1091a) | `tu` | `head_graph_oc_bs` |
| `13-22` | 鑄→tu [*tu-s F!] (1090a'); 鑄→ku [*klʲus] (None); 鑄→ɡlu [*ɡlu] (None); 壽→ɡlʲu [*ɡlʲuʔ] (None) | `ɡlu` | `head_graph_supported_root` |
| `13-23` | 肘→ku [*t-k<r>uʔ] (1073a) | `ku` | `head_graph_oc_bs` |
| `13-24` | 盩→tu [*tu] (None) | `tu` | `head_graph_oc_shengfu` |
| `13-25` | 鳥→tiw [*ti̠wʔ] (None) | `tiw` | `head_graph_oc_shengfu` |
| `13-27` | 舀→lo [*ɡ‧lo] (None); 舀→liw [*ɡ‧liwʔ] (None) | `lu` | `packet_bs_consensus` |
| `13-30` | 由→lu [*lu {*l[u]}] (1079a) | `lu` | `head_graph_oc_bs` |
| `13-31` | 牖→lu [*luʔ {*[l]uʔ}] (1082a) | `lu` | `head_graph_oc_bs` |
| `13-32` | 攸→liw [*liw] (1077a) | `liw` | `head_graph_oc_bs` |
| `13-33` | 斿→lu [*lu] (None) | `lu` | `head_graph_oc_shengfu` |
| `13-34` | 卣→lu [*lu] (None) | `lu` | `head_graph_oc_shengfu` |
| `13-35` | 囚→lu [*sə.lu] (1094a) | `lu` | `head_graph_oc_bs` |
| `13-36` | 酉→ru [*m.ruʔ {*[m].ruʔ}] (1096a) | `ru` | `head_graph_oc_bs` |
| `13-37` | 狖→lu [*ɡ‧lus] (None); 狖→lum [*k‧lum] (None) | `lu` | `head_graph_shengfu_head` |
| `13-38` | 首→lu [*l̥uʔ-s] (1102a) | `lu` | `head_graph_oc_bs` |
| `13-39` | 秀→lu [*s-lu-s {*[s-l]u(ʔ)-s}] (1095a) | `lu` | `head_graph_oc_bs` |
| `13-40` | 守→tu [*s.tuʔ-s] (1099a) | `tu` | `head_graph_oc_bs` |
| `13-41` | 獸→tu [*s.tu-s {*s.tu(ʔ)-s}] (1100a) | `tu` | `head_graph_oc_bs` |
| `13-42` | 手→nu [*n̥uʔ] (1101a) | `nu` | `head_graph_oc_bs` |
| `13-43` | 老→ru [*C-rˤuʔ] (1055a) | `ru` | `head_graph_oc_bs` |
| `13-44` | 牢→ru [*ɡ‧ru̠] (None) | `ru` | `head_graph_oc_shengfu` |
| `13-45` | 翏→riw [*riw-s {*[r]iw-s}] (1069a) | `riw` | `head_graph_oc_bs` |
| `13-46` | 流→ru [*ru {*[r]u}] (1104a) | `ru` | `head_graph_oc_bs` |
| `13-47` | 卯→mu [*mˤruʔ] (1114a) | `mu` | `head_graph_oc_bs` |
| `13-48` | 柔→nu [*nu] (1105a) | `nu` | `head_graph_oc_bs` |
| `13-50` | 丑→nu [*n̥ruʔ {*[n̥]ruʔ}] (1076a) | `nu` | `head_graph_oc_bs` |
| `13-51` | 早→tsu [*Nə.tsˤuʔ {*Nə.[ts]ˤuʔ}] (1049a) | `tsu` | `head_graph_oc_bs` |
| `13-52` | 屮→kat [*kʰlad] (None) | `kat` | `head_graph_oc_shengfu` |
| `13-53` | 棗→tsu [*tsˤuʔ {*[ts]ˤuʔ}] (1050a) | `tsu` | `head_graph_oc_bs` |
| `13-54` | 造→tsu [*Cə.dzˤuʔ] (1051a) | `tsu` | `head_graph_oc_bs` |
| `13-55` | 曹→tsu [*N-tsˤu] (1053a) | `tsu` | `head_graph_oc_bs` |
| `13-56` | 皂→su [*zu̠ʔ] (None); 皂→tsu [*Nə.tsˤuʔ {*Nə.[ts]ˤuʔ}] (None) | `su` | `head_graph_shengfu_head` |
| `13-57` | 秋→tsiw [*tsʰiw] (1092a) | `tsiw` | `head_graph_oc_bs` |
| `13-58` | 就→tsuk [*dzuk-s {*[dz]u[k]-s}] (1093a) | `tsuk` | `head_graph_oc_bs` |
| `13-59` | 爪→tsu [*[ts]ˤruʔ] (1111a) | `tsu` | `head_graph_oc_bs` |
| `13-60` | 㕚→su [*sru̠ʔ] (None); 㕚→tsu [*[ts]ˤruʔ] (None) | `su` | `head_graph_shengfu_head` |
| `13-61` | 帚→py [*t.pəʔ {*[t.p]əʔ}] (1087a) | `py` | `head_graph_oc_bs` |
| `13-62` | 蒐→su [*skʳu] (None); 蒐→ʔuy [*k-ʔujʔ] (None) | `su` | `head_graph_shengfu_head` |
| `13-64` | 保→pu [*pˤuʔ] (1057a) | `pu` | `head_graph_oc_bs` |
| `13-65` | 報→puk [*pˤuk-s] (1058a) | `puk` | `head_graph_oc_bs` |
| `13-66` | 寶→pu [*pˤuʔ] (1059a) | `pu` | `head_graph_oc_bs` |
| `13-67` | 鴇→pu [*pu̠ʔ] (None) | `pu` | `head_graph_oc_shengfu` |
| `13-68` | 虣→pok [*bo̠ɡʷs] (None); 虣→pak [*bˤawk-s {*[b]ˤawk-s}] (None) | `pok` | `head_graph_shengfu_head` |
| `13-69` | 缶→pu [*puʔ {*p(r)uʔ}] (1107a) | `pu` | `head_graph_oc_bs` |
| `13-70` | 阜→pu [*buʔ {*[b](r)uʔ}] (1108a) | `pu` | `head_graph_oc_bs` |
| `13-71` | 髟→sam [*sˤram {*s[ˤ]ram}] (1154a) | `sam` | `head_graph_oc_bs` |
| `13-72` | 包→pu [*pˤ<r>u] (1113a) | `pu` | `head_graph_oc_bs` |
| `13-73` | 孚→pu [*pʰu] (None) | `pu` | `head_graph_oc_shengfu` |
| `13-74` | 冒→muk [*mˤuk-s] (1062a) | `muk` | `head_graph_oc_bs` |
| `13-75` | 牡→mu [*muʔ] (1063a) | `mu` | `head_graph_oc_bs` |
| `13-76` | 矛→mu [*mu {*m(r)u}] (1109a) | `mu` | `head_graph_oc_bs` |
| `13-77` | 牟→mu [*mu] (None) | `mu` | `head_graph_oc_shengfu` |
| `13-78` | 戊→mu [*muʔ-s {*m(r)uʔ-s} ~ *m(r)uʔ] (1231a) | `mu` | `head_graph_oc_bs` |
| `14-02` | 匊→kuk [*kuɡ] (None) | `kuk` | `head_graph_oc_shengfu` |
| `14-03` | 學→kuk [*m-kˤruk] (1038a) | `kuk` | `head_graph_oc_bs` |
| `14-04` | 奧→ʔuk [*ʔˤuk-s] (1045a) | `ʔuk` | `head_graph_oc_bs` |
| `14-05` | 毒→tuk [*dˤuk {*[d]ˤuk}] (1016a) | `tuk` | `head_graph_oc_bs` |
| `14-06` | 祝→tuk [*tuk-s] (1025a) | `tuk` | `head_graph_oc_bs` |
| `14-07` | 孰→tuk [*duk {*[d]uk}] (1026a) | `tuk` | `head_graph_oc_bs` |
| `14-08` | 竹→tuk [*truk] (1019a) | `tuk` | `head_graph_oc_bs` |
| `14-09` | 晝→tu [*tus] (None) | `tu` | `head_graph_oc_shengfu` |
| `14-10` | 粥→quk [*t-quk {*[t-q]uk}] (1024a) | `quk` | `head_graph_oc_bs` |
| `14-11` | 育→quk [*m-quk] (1020a) | `quk` | `head_graph_oc_bs` |
| `14-12` | 毓→quk [*m-quk] (1021a) | `quk` | `head_graph_oc_bs` |
| `14-13` | 逐→lik [*lriwk {*[l]riwk}] (1022a) | `lik` | `head_graph_oc_bs` |
| `14-14` | 賣→me [*mʳe̠s] (None); 賣→ray [*C.raj-s] (None) | `me` | `head_graph_shengfu_head` |
| `14-15` | 畜→quk [*qʰ<r>uk-s] (1018a) | `quk` | `head_graph_oc_bs` |
| `14-16` | 六→ruk [*k.ruk] (1032a) | `ruk` | `head_graph_oc_bs` |
| `14-17` | 肉→nuk [*k.nuk] (1033a) | `nuk` | `head_graph_oc_bs` |
| `14-18` | 尗→tuk [*s.tuk] (1031a) | `tuk` | `head_graph_oc_bs` |
| `14-19` | 鼀→suk [*skʰluɡ] (None) | `suk` | `head_graph_oc_shengfu` |
| `14-20` | 宿→suk [*suk-s {*[s]uk-s}] (1029a) | `suk` | `head_graph_oc_bs` |
| `14-21` | 夙→suk [*suk {*[s]uk}] (1030a) | `suk` | `head_graph_oc_bs` |
| `14-22` | 肅→sik [*siwk] (1028a) | `sik` | `head_graph_oc_bs` |
| `14-23` | 复→puk [*m-puk {*m-p(r)uk}] (1034a) | `puk` | `head_graph_oc_bs` |
| `14-24` | 目→muk [*C.muk {*C.m(r)[u]k}] (1036a) | `muk` | `head_graph_oc_bs` |
| `14-25` | 穆→mik [*miwk {*m(r)iwk}] (1035a) | `mik` | `head_graph_oc_bs` |
| `15-01` | 宮→kuṅ [*kuŋ {*k(r)uŋ}] (1006a) | `kuṅ` | `head_graph_oc_bs` |
| `15-02` | 降→kum [*m-kˤruŋ (? < *-[u]m)] (1015a) | `kum` | `head_graph_oc_bs` |
| `15-03` | 冬→tuṅ [*tˤuŋ] (1002a) | `tuṅ` | `head_graph_oc_bs` |
| `15-04` | 眾→tuṅ [*tuŋ-s] (1010a) | `tuṅ` | `head_graph_oc_bs` |
| `15-05` | 充→tuṅ [*tʰuŋ] (1011a) | `tuṅ` | `head_graph_oc_bs` |
| `15-06` | 中→tuṅ [*truŋ] (1007a) | `tuṅ` | `head_graph_oc_bs` |
| `15-07` | 蟲→luṅ [*C.lruŋ] (1009a) | `luṅ` | `head_graph_oc_bs` |
| `15-08` | 肜→ruṅ [*m.ruŋ {*m.rum} (dial. *-um > -uŋ)] (1008a) | `ruṅ` | `head_graph_oc_bs` |
| `15-09` | 農→noṅ [*nˤoŋ {*nˤ[o]ŋ}] (1005a) | `noṅ` | `head_graph_oc_bs` |
| `15-10` | 戎→nuṅ [*nuŋ] (1013a) | `nuṅ` | `head_graph_oc_bs` |
| `15-11` | 嵩→suṅ [*sˡuŋ] (None) | `suṅ` | `head_graph_oc_shengfu` |
| `15-12` | 宋→sum [*sqˡu̠ms] (None) | `sum` | `head_graph_oc_shengfu` |
| `15-13` | 宗→tsuṅ [*tsˤuŋ {*[ts]ˤuŋ}] (1003a) | `tsuṅ` | `head_graph_oc_bs` |
| `15-14` | 豐→poṅ [*pʰoŋ {*pʰ(r)oŋ}] (1014a) | `poṅ` | `head_graph_oc_bs` |
| `15-15` | 賵→muṅ [*m̥uŋs] (None); 賵→muk [*mˤuk-s] (None) | `muṅ` | `head_graph_mc_coda` |
| `16-01` | 高→kaw [*Cə.kˤaw {*Cə.[k]ˤaw}] (1129a) | `kaw` | `head_graph_oc_bs` |
| `16-02` | 羔→ku [*ku̠] (None) | `ku` | `head_graph_oc_shengfu` |
| `16-03` | 喬→kaw [*N-kaw {*[N-k](r)aw}] (1138a) | `kaw` | `head_graph_oc_bs` |
| `16-06` | 交→kaw [*kˤraw {*[k]ˤraw}] (1166a) | `kaw` | `head_graph_oc_bs` |
| `16-07` | 爻→kaw [*N-kˤraw] (1167a) | `kaw` | `head_graph_oc_bs` |
| `16-08` | 号→ku [*ku̠] (None); 號→kaw [*C.gˤaw {*[C.g]ˤaw}] (1041q); 號→qu [*ɢu̠] (None); 呺→qiw [*qʰʳiw] (None); 鴞→quaw [*ɢʷaw {*[ɢ]ʷ(r)aw}] (1041t); 𧴪→so [*sˡo̠lʔ] (None); 鎖→soy [*sˤojʔ {*[s]ˤojʔ}] (13) | `ku` | `head_graph_supported_root` |
| `16-09` | 孝→qu [*qʰˤ<r>u-s {*qʰˤ<r>uʔ-s}] (1168a); 沙→say [*sˤraj {*[s]ˤraj}] (0016a) | `qu` | `merged_packet_primary_head` |
| `16-10` | 夭→ʔaw [*ʔaw {*[ʔ](r)aw}] (1141a) | `ʔaw` | `head_graph_oc_bs` |
| `16-11` | 要→ʔew [*ʔew-s] (1142a) | `ʔew` | `head_graph_oc_bs` |
| `16-12` | 囂→ṅow [*ŋo̠w] (None) | `ṅow` | `head_graph_oc_shengfu` |
| `16-13` | 敖→ṅow [*ŋɡo̠w] (None) | `ṅow` | `head_graph_oc_shengfu` |
| `16-14` | 堯→ṅew [*ŋˤew {*[ŋ]ˤew}] (1164a) | `ṅew` | `head_graph_oc_bs` |
| `16-15` | 刀→taw [*C.tˤaw] (1131a) | `taw` | `head_graph_oc_bs` |
| `16-16` | 盜→taw [*dˤaw-s {*[d]ˤaw(k)-s}] (1133a) | `taw` | `head_graph_oc_bs` |
| `16-17` | 朝→taw [*m-t<r>aw {*[m]-t<r>aw}] (1143a) | `taw` | `head_graph_oc_bs` |
| `16-18` | 肇→law [*lrawʔ] (None) | `law` | `same_character_series_root` |
| `16-19` | 鼂→tew [*dew] (None) | `tew` | `head_graph_oc_shengfu` |
| `16-20` | 兆→law [*lrawʔ {*lr[a]wʔ}] (1145a) | `law` | `head_graph_oc_bs` |
| `16-21` | 䍃→law [*law] (1144a) | `law` | `head_graph_oc_bs` |
| `16-22` | 少→tew [*s-tewʔ-s {*[s-t]ewʔ-s}] (1149e); 少→qew [*qʰlewʔ] (None); 少→sew [*sewʔ] (None) | `sew` | `head_graph_supported_root` |
| `16-23` | 勞→raw [*rˤaw {*[r]ˤaw}] (1135a) | `raw` | `head_graph_oc_bs` |
| `16-24` | 𤋯→rew [*ɡ‧re̠w] (None) | `rew` | `same_character_series_root` |
| `16-26` | 窅→qew [*qʳe̠w] (None) | `qew` | `head_graph_oc_shengfu` |
| `16-27` | 擾→miw [*mɡlʲiwʔ] (None); 𧴪→so [*sˡo̠lʔ] (None) | `miw` | `head_graph_supported_root` |
| `16-28` | 羔→ku [*ku̠] (None) | `ku` | `head_graph_oc_shengfu` |
| `16-29` | 羔→ku [*ku̠] (None) | `ku` | `head_graph_oc_shengfu` |
| `16-30` | 喿→saw [*sˡa̠ws] (None) | `saw` | `head_graph_oc_shengfu` |
| `16-31` | 焦→sew [*skew] (None) | `sew` | `head_graph_oc_shengfu` |
| `16-32` | 巢→tsaw [*dzˤraw {*[dz]ˤraw}] (1169a) | `tsaw` | `head_graph_oc_bs` |
| `16-33` | 小→sew [*sewʔ {*[s]ewʔ}] (1149a) | `sew` | `head_graph_oc_bs` |
| `16-34` | 笑→law [*s-law-s] (1150a) | `law` | `head_graph_oc_bs` |
| `16-35` | 表→paw [*pawʔ {*p(r)awʔ}] (1153a) | `paw` | `head_graph_oc_bs` |
| `16-36` | 髟→sam [*sˤram {*s[ˤ]ram}] (1154a) | `sam` | `head_graph_oc_bs` |
| `16-37` | 猋→pew [*pew] (None) | `pew` | `head_graph_oc_shengfu` |
| `16-38` | 驫→pew [*pew] (None); 驫→piw [*pʳiw] (None) | `piw` | `head_graph_supported_root` |
| `16-39` | 𤐫→pew [*pʰew] (None) | `pew` | `head_graph_oc_shengfu` |
| `16-40` | 麃→pew [*bʳe̠w] (None) | `pew` | `head_graph_oc_shengfu` |
| `16-41` | 毛→maw [*C.mˤaw] (1137a) | `maw` | `head_graph_oc_bs` |
| `16-42` | 皃→mek [*mʳe̠ɡʷ] (None) | `mek` | `head_graph_oc_shengfu` |
| `16-43` | 苗→maw [*maw {*m(r)aw}] (1159a) | `maw` | `head_graph_oc_bs` |
| `16-44` | 眇→mew [*mewʔ] (1158a) | `mew` | `head_graph_oc_bs` |
| `16-45` | 淼→mew [*mewʔ] (None) | `mew` | `head_graph_oc_shengfu` |
| `17-01` | 隺→kak [*gˤawk {*[g]ˤawk}] (1117a) | `kak` | `head_graph_oc_bs` |
| `17-02` | 虐→ṅak [*ŋawk {*[ŋ](r)awk}] (1118a) | `ṅak` | `head_graph_oc_bs` |
| `17-03` | 弔→tek [*tˤewk {*tˤ[e]wk}] (1165a) | `tek` | `head_graph_oc_bs` |
| `17-05` | 勺→qek [*m-t-qewk] (1120a) | `qek` | `head_graph_oc_bs` |
| `17-06` | 龠→lek [*lewk] (1119a) | `lek` | `head_graph_oc_bs` |
| `17-07` | 翟→lek [*lˤewk] (1124a) | `lek` | `head_graph_oc_bs` |
| `17-08` | 樂→rak [*rˤawk {*[rˤ]awk}] (1125a); 樂→ṅak [*ŋˤrawk {*[ŋ]ˤrawk}] (1125a) | `ṅak` | `head_graph_mc_onset` |
| `17-09` | 弱→nek [*newk] (1123a) | `nek` | `head_graph_oc_bs` |
| `17-11` | 鑿→tsak [*dzˤawk {*[dz]ˤawk}] (1128a) | `tsak` | `head_graph_oc_bs` |
| `17-12` | 爵→tsek [*tsewk {*[ts]ewk}] (1121a) | `tsek` | `head_graph_oc_bs` |
| `17-13` | 雀→tsek [*tsewk {*[ts]ewk}] (1122a) | `tsek` | `head_graph_oc_bs` |
| `17-14` | 暴→pak [*bˤawk-s {*[b]ˤawk-s}] (1136a) | `pak` | `head_graph_oc_bs` |
| `17-15` | 駁→puk [*pkʳu̠ɡ] (None) | `puk` | `head_graph_oc_shengfu` |
| `17-16` | 豹→pek [*pʳe̠ɡʷs] (None) | `pek` | `head_graph_oc_shengfu` |
| `18-02` | 个→kar [*kˤar-s {*kˤa[r]-s}] (0349a) | `kar` | `head_graph_oc_bs` |
| `18-10` | 羅→ray [*rˤaj] (0006a) | `ray` | `head_graph_oc_bs` |
| `18-12` | 那→na [*na̠l] (None); 那→nam [*nʲam] (None) | `na` | `head_graph_shengfu_head` |
| `18-17` | 罷→pay [*bˤrajʔ {*[b]ˤrajʔ}] (0026a) | `pay` | `head_graph_oc_bs` |
| `18-19` | 毀→may [*m̥ajʔ {*[m̥](r)ajʔ}] (0356a) | `may` | `head_graph_oc_bs` |
| `19-02` | 果→kor [*kˤorʔ {*[k]ˤo[r]ʔ}] (0351a) | `kor` | `head_graph_oc_bs` |
| `19-03` | 𩰬→kuay [None] (None) | `kuay` | `same_character_series_root` |
| `19-09` | 隓→loy [*lˤojʔ] (0011a) | `loy` | `head_graph_oc_bs` |
| `19-10` | 委→qoy [*qojʔ {*q(r)ojʔ}] (0357a); 委→qor [*qorʔ {*q(r)orʔ}] (0357a) | `qoy` | `head_graph_supported_root` |
| `19-19` | 妥→noy [*n̥ˤojʔ] (0354a) | `noy` | `head_graph_oc_bs` |
| `19-23` | 衰→suy [*sruj] (0355a) | `suy` | `head_graph_oc_bs` |
| `20-01` | 㓞→ket [*kʰʳe̠d] (None) | `ket` | `head_graph_oc_shengfu` |
| `20-02` | 介→kep [*kˤrep-s {*kˤr[e][p]-s}] (0327a) | `kep` | `head_graph_oc_bs` |
| `20-03` | 夬→kuet [*kʷˤret-s {*[k]ʷˤret-s}] (0312a) | `kuet` | `head_graph_oc_bs` |
| `20-04` | 軋→qyt [*qˤrət] (0280a) | `qyt` | `head_graph_oc_bs` |
| `20-05` | 隉→ṅet [*ŋɡe̠d] (None); 隉→ṅe [*ŋe] (None) | `ṅet` | `head_graph_mc_coda` |
| `20-06` | 帠→ṅy [None] (None) | `ṅy` | `same_character_series_root` |
| `20-07` | 臬→ṅet [*ŋe̠d] (None) | `ṅet` | `head_graph_oc_shengfu` |
| `20-08` | 設→ṅet [*ŋ̊et] (0290a) | `ṅet` | `head_graph_oc_bs` |
| `20-09` | 隉→ṅet [*ŋɡe̠d] (None) | `ṅet` | `head_graph_oc_shengfu` |
| `20-10` | 舌→lat [*mə.lat] (0288a) | `lat` | `head_graph_oc_bs` |
| `20-11` | 竊→tset [*tsʰˤet {*[tsʰ]ˤet}] (0309a) | `tset` | `head_graph_oc_bs` |
| `20-12` | 𢧵→set [*ze̠d] (None) | `set` | `same_character_series_root` |
| `20-13` | 埶→ṅe [*ŋet-s I!] (0330a); 埶→ṅet [*ŋ̊et-s] (0330a) | `ṅet` | `packet_shengfu_majority` |
| `20-14` | 八→pet [*pˤret] (0281a) | `pet` | `head_graph_oc_bs` |
| `20-15` | 別→pet [*N-pret] (0292a) | `pet` | `head_graph_oc_bs` |
| `20-16` | 敝→pet [*bet-s {*[b]e[t]-s}] (0341a) | `pet` | `head_graph_oc_bs` |
| `20-17` | 拜→pot [*C.pˤrot-s {*C.pˤro[t]-s}] (0328a) | `pot` | `head_graph_oc_bs` |
| `20-18` | 蔑→met [*mˤet] (0311a) | `met` | `head_graph_oc_bs` |
| `20-19` | 烕→met [*m̥et] (0294a) | `met` | `head_graph_oc_bs` |
| `21-01` | 匃→kat [*kˤat-s {*[k]ˤat-s}] (0313a) | `kat` | `head_graph_oc_bs` |
| `21-02` | 害→kat [*N-kˤat-s] (0314a) | `kat` | `head_graph_oc_bs` |
| `21-03` | 舝→ɡat [*ɡˤrat-s] (None) | `ɡat` | `same_character_series_root` |
| `21-04` | 孑→ket [*ked] (None) | `ket` | `head_graph_oc_shengfu` |
| `21-05` | 桀→kat [*m-k<r>at {*m-[k]<r>at}] (0284a) | `kat` | `head_graph_oc_bs` |
| `21-06` | 憩→kat [*kʰʳads] (None) | `kat` | `head_graph_oc_shengfu` |
| `21-07` | 瘞→qep [*qʳebs] (None); 瘞→kep [*kʳe̠b] (None) | `qep` | `head_graph_shengfu_head` |
| `21-08` | 枿→ṅat [*ŋa̠d] (None) | `ṅat` | `head_graph_oc_shengfu` |
| `21-09` | 歺→ṅat [*ŋa̠d] (None) | `ṅat` | `head_graph_oc_shengfu` |
| `21-10` | 乂→ṅat [*ŋat-s {*ŋa[t]-s}] (0347a) | `ṅat` | `head_graph_oc_bs` |
| `21-11` | 辥→ṅet₈ [*sŋed] (None) | `ṅet₈` | `same_character_series_root` |
| `21-12` | 大→la [*lˤat-s (F!)] (0317a); 大→lat [*lˤat-s {*lˤa[t]-s}] (0317a) | `lat` | `packet_bs_majority` |
| `21-13` | 泰→lat [*l̥ˤat-s {*l̥ˤa[t]-s}] (0316a) | `lat` | `head_graph_oc_bs` |
| `21-14` | 𦍒→lat [*l̥ˤat] (0271a) | `lat` | `head_graph_oc_bs` |
| `21-15` | 帶→tat [*C.tˤat-s {*C.tˤa[t]-s}] (0315a) | `tat` | `head_graph_oc_bs` |
| `21-16` | 筮→tet [*dʲeds] (None) | `tet` | `head_graph_oc_shengfu` |
| `21-17` | 制→tet [*tet-s] (0335a) | `tet` | `head_graph_oc_bs` |
| `21-18` | 彘→lat [*lrat-s {*[l]r[a][t]-s}] (0334a) | `lat` | `head_graph_oc_bs` |
| `21-19` | 折→tet [*dˤet-s {*[d]ˤet-s}] (0287a) | `tet` | `head_graph_oc_bs` |
| `21-20` | 撤→tet [*m-tʰret {*m-tʰr[e]t}] (0286b) | `tet` | `head_graph_oc_bs` |
| `21-21` | 曳→let [*leds] (None) | `let` | `head_graph_oc_shengfu` |
| `21-22` | 裔→let [*leds] (None) | `let` | `head_graph_oc_shengfu` |
| `21-23` | 世→lap [*l̥ap-s {*l̥[a]p-s}] (0339a) | `lap` | `head_graph_oc_bs` |
| `21-24` | 剌→ra [*rˤat (~ C.rˤat ?)] (0272a) | `ra` | `head_graph_oc_bs` |
| `21-25` | 列→ret [*ret {*[r][e]t}] (0291a) | `ret` | `head_graph_oc_bs` |
| `21-26` | 蠆→myt [*mə-r̥ˤat-s {*mə-r̥ˤa[t]-s}] (0326a); 厲→rat [*rat-s {*[r]at-s}] (0340a); 萬→man [*C.man-s {*C.ma[n]-s}] (0267a) | `myt` | `merged_packet_primary_head` |
| `21-27` | 柰→nat [*na̠ds] (None) | `nat` | `head_graph_oc_shengfu` |
| `21-28` | 祭→tset [*tset-s {*[ts]et-s}] (0337a) | `tset` | `head_graph_oc_bs` |
| `21-29` | 杀→sat [*s<r>at-s] (None) | `sat` | `same_character_series_root` |
| `21-30` | 癶→pat [*pa̠d] (None) | `pat` | `head_graph_oc_shengfu` |
| `21-31` | 犮→pot [*bo̠d] (None) | `pot` | `head_graph_oc_shengfu` |
| `21-33` | 伐→pat [*m-pat] (0307a) | `pat` | `head_graph_oc_bs` |
| `21-34` | 罰→pat [*bat {*[b][a]t}] (0308a) | `pat` | `head_graph_oc_bs` |
| `21-35` | 貝→pat [*pa̠ds] (None) | `pat` | `head_graph_oc_shengfu` |
| `21-36` | 吠→pot [*Cə.bot-s {*Cə.[b]o[t]-s}] (0348a) | `pot` | `head_graph_oc_bs` |
| `21-37` | 末→mat [*mˤat] (0277a) | `mat` | `head_graph_oc_bs` |
| `21-38` |  | `` | `` |
| `22-01` | 氒→kot [*kod] (None) | `kot` | `head_graph_oc_shengfu` |
| `22-02` | 欮→kot [*kod] (None) | `kot` | `head_graph_oc_shengfu` |
| `22-03` | 會→kop [*m-kˤop-s] (0321a) | `kop` | `head_graph_oc_bs` |
| `22-04` | 曰→quat [*ɢʷat {*[ɢ]ʷat}] (0304a) | `quat` | `head_graph_oc_bs` |
| `22-05` | 戉→quat [*ɢʷat {*[ɢ]ʷat}] (0303a) | `quat` | `head_graph_oc_bs` |
| `22-06` | 粵→quat [*ɢʷat {*[ɢ]ʷat}] (0305a) | `quat` | `head_graph_oc_bs` |
| `22-07` | 𥄗→qot [*qo̠d] (None) | `qot` | `head_graph_oc_shengfu` |
| `22-08` | 月→ṅat [*ŋʷat {*[ŋ]ʷat}] (0306a) | `ṅat` | `head_graph_oc_bs` |
| `22-09` | 奪→lot [*Cə.lˤot] (0274a) | `lot` | `head_graph_oc_bs` |
| `22-10` | 叕→tot [*trot-s] (0295a) | `tot` | `head_graph_oc_bs` |
| `22-11` | 贅→kot [*klʲods] (None); 贅→tot [*trot-s] (None) | `kot` | `head_graph_shengfu_head` |
| `22-12` | 叡→qot [*ɢods] (None) | `qot` | `head_graph_oc_shengfu` |
| `22-13` | 兌→lot [*lˤot-s] (0324a) | `lot` | `head_graph_oc_bs` |
| `22-14` | 寽→rut [*rud] (None) | `rut` | `head_graph_oc_shengfu` |
| `22-16` | 絕→tsot [*dzot {*[dz]ot}] (0296a) | `tsot` | `head_graph_oc_bs` |
| `22-17` | 毳→tsop [*tsʰop-s {*[tsʰ]op-s}] (0345a) | `tsop` | `head_graph_oc_bs` |
| `22-18` | 雪→sot [*sot {*[s]ot}] (0297a) | `sot` | `head_graph_oc_bs` |
| `22-19` | 刷→sot [*srot {*[s]rot} (dial. retains srj-)] (0298a) | `sot` | `head_graph_oc_bs` |
| `23-01` | 肩→ken [*kˤen {*[k]ˤe[n]}] (0240a) | `ken` | `head_graph_oc_bs` |
| `23-02` | 見→ken [*N-kˤen-s {*N-[k]ˤen-s}] (0241a) | `ken` | `head_graph_oc_bs` |
| `23-03` | 幵→ker [*kˤer {*[k]ˤer}] (0239a) | `ker` | `head_graph_oc_bs` |
| `23-04` | 遣→ken [*kʰenʔ-s {*[k]ʰe[n]ʔ-s} (*kʰ- doesn't palatalize)] (0196b) | `ken` | `head_graph_oc_bs` |
| `23-05` | 閒→ken [*kʳe̠n] (None) | `ken` | `head_graph_oc_shengfu` |
| `23-06` | 閑→kan [*gˤran {*[g]ˤra[n]}] (0192a); 閑→ken [*N-kˤren {*N-kˤre[n]}] (0192a) | `kan` | `head_graph_supported_root` |
| `23-07` | 柬→kan [*kˤranʔ {*kˤr[a]nʔ}] (0185a) | `kan` | `head_graph_oc_bs` |
| `23-08` | 蠲→kuiṅ [*kʷi̠ŋ] (None); 蠲→mik [*m̥ik {*m̥(r)ik}] (None) | `kuiṅ` | `head_graph_shengfu_head` |
| `23-09` | 縣→kuen [*gʷˤen {*[g]ʷˤe[n]}] (0248a) | `kuen` | `head_graph_oc_bs` |
| `23-10` | 員→quyn [*ɢʷən {*[ɢ]ʷə[n]}] (0227a); 員→quen [*ɢʷren {*[ɢ]ʷre[n]}] (0227a) | `qun` | `packet_shengfu_majority` |
| `23-11` | 瞏→kueṅ [*gʷeŋ {*[g]ʷeŋ}] (0829a) | `kueṅ` | `head_graph_oc_bs` |
| `23-12` | 奐→qon [*qʰo̠ns] (None) | `qon` | `head_graph_oc_shengfu` |
| `23-13` | 幻→ɡin [*ɡʷʳi̠n] (None) | `ɡin` | `head_graph_oc_shengfu` |
| `23-14` | 旋→quen [*s-ɢʷen {*s-[ɢ]ʷe[n]}] (0236a) | `quen` | `head_graph_oc_bs` |
| `23-15` | 燕→ʔen [*ʔˤen-s {*ʔˤe[n]-s}] (0243a) | `ʔen` | `head_graph_oc_bs` |
| `23-16` | 顯→qen [*qʰˤenʔ] (0242a) | `qen` | `head_graph_oc_bs` |
| `23-17` | 肙→quen [*qʷˤen] (0228a) | `quen` | `head_graph_oc_bs` |
| `23-18` | 靦→tyn [*tʰɯ̠nʔ] (None) | `tyn` | `head_graph_oc_shengfu` |
| `23-19` | 晛→ɡen [*ɡe̠nʔ] (None); 晛→ken [*N-kˤen-s {*N-[k]ˤen-s}] (None); 晛→men [*mɡle̠ns] (None) | `ken` | `head_graph_supported_root` |
| `23-20` | 前→tsen [*dzˤen ~ *m-dzˤen] (0245a) | `tsen` | `head_graph_oc_bs` |
| `23-21` | 鮮→ser [*ser {*[s][e]r}] (0209a) | `ser` | `head_graph_oc_bs` |
| `23-22` | 鱻→len [*l̥ʲen] (None) | `len` | `head_graph_oc_shengfu` |
| `23-23` | 尟→sen [*sˡenʔ] (None) | `sen` | `head_graph_oc_shengfu` |
| `23-24` | 片→pen [*pʰˤen-s {*pʰˤe[n]-s}] (0218a) | `pen` | `head_graph_oc_bs` |
| `23-25` | 便→pen [*ben {*[b]e[n]}] (0221a) | `pen` | `head_graph_oc_bs` |
| `23-26` | 臱→men [*mben] (None) | `men` | `head_graph_oc_shengfu` |
| `23-27` | 扁→pen [*pˤenʔ {*pˤe[n]ʔ}] (0246a) | `pen` | `head_graph_oc_bs` |
| `23-28` | 辡→pen [*bʳenʔ] (None) | `pen` | `head_graph_oc_shengfu` |
| `23-29` | 綿→men [*men] (None) | `men` | `same_character_series_root` |
| `23-31` | 面→men [*C.men-s {*C.me[n]-s}] (0223a) | `men` | `head_graph_oc_bs` |
| `23-32` | 丏→men [*mbe̠nʔ] (None) | `men` | `head_graph_oc_shengfu` |
| `24-01` | 干→kar [*kˤar {*kˤa[r]}] (0139a) | `kar` | `head_graph_oc_bs` |
| `24-02` | 倝→kan [*ka̠ns] (None) | `kan` | `head_graph_oc_shengfu` |
| `24-03` | 侃→kan [*kʰa̠nʔ] (None); 𩱡→qan [*C.qan {*C.qa[n]}] (141) | `qan` | `head_graph_bs_head` |
| `24-05` | 寒→kan [*Cə.gˤan {*Cə.[g]ˤa[n]}] (0143a) | `kan` | `head_graph_oc_bs` |
| `24-07` | 虔→kan [*gran {*[g]ra[n]}] (0198a) | `kan` | `head_graph_oc_bs` |
| `24-08` | 建→kan [*kan-s {*[k]a[n]-s}] (0249a) | `kan` | `head_graph_oc_bs` |
| `24-09` | 姦→kan [*kˤran {*[k]ˤran}] (0184a) | `kan` | `head_graph_oc_bs` |
| `24-10` | 暵→qar [*qʰˤar-s {*[qʰ]ˤarʔ-s}] (0144a) | `qar` | `head_graph_oc_bs` |
| `24-11` | 安→ʔan [*ʔˤan {*[ʔ]ˤa[n]}] (0146a) | `ʔan` | `head_graph_oc_bs` |
| `24-12` | 妟→qan [*qʳa̠ns] (None); 妟→qen [*qe̠nʔ] (None) | `qen` | `head_graph_supported_root` |
| `24-13` | 㫃→qan [*qanʔ] (None) | `qan` | `head_graph_oc_shengfu` |
| `24-14` | 焉→ʔan [*ʔan {*ʔa[n]}] (0200a) | `ʔan` | `head_graph_oc_bs` |
| `24-15` | 厂→ṅar [*ŋ̊ˤar-s {*ŋ̊ˤar(ʔ)-s}] (0145a) | `ṅar` | `head_graph_oc_bs` |
| `24-16` | 言→ṅan [*ŋan {*ŋa[n]}] (0251a) | `ṅan` | `head_graph_oc_bs` |
| `24-17` | 鬳→ṅar [*ŋar-s] (0252a) | `ṅar` | `head_graph_oc_bs` |
| `24-18` | 憲→qan [*qʰans] (None); 憲→kat [*N-kˤat-s] (None) | `qan` | `head_graph_mc_coda` |
| `24-19` | 彥→ṅen [*ŋʳens] (None); 彥→ṅar [*ŋ̊ˤar-s {*ŋ̊ˤar(ʔ)-s}] (None) | `ṅen` | `head_graph_mc_coda` |
| `24-20` | 丹→tan [*tˤan] (0150a) | `tan` | `head_graph_oc_bs` |
| `24-21` | 單→tar [*dˤar {*[d]ˤar}] (0147a) | `tar` | `head_graph_oc_bs` |
| `24-22` | 旦→tan [*tˤan-s {*tˤa[n]-s}] (0149a) | `tan` | `head_graph_oc_bs` |
| `24-23` | 亶→tan [*tˤanʔ {*tˤa[n]ʔ}] (0148a) | `tan` | `head_graph_oc_bs` |
| `24-24` | 炭→tan [*tʰˤan-s {*[tʰ]ˤa[n]-s}] (0151a) | `tan` | `head_graph_oc_bs` |
| `24-25` | 善→ken [*genʔ {*[g]e[n]ʔ}] (0205a) | `ken` | `head_graph_oc_bs` |
| `24-26` | 展→ten [*trenʔ] (0201a) | `ten` | `head_graph_oc_bs` |
| `24-27` | 蕆→tan [*tʰanʔ] (None) | `tan` | `head_graph_oc_shengfu` |
| `24-28` | 廛→tan [*dran {*[d]ra[n]}] (0204a) | `tan` | `head_graph_oc_bs` |
| `24-29` | 衍→qan [*N-qanʔ {*N-q(r)anʔ}] (0197a) | `qan` | `head_graph_oc_bs` |
| `24-30` | 延→lan [*lan {*la[n]}] (0203a) | `lan` | `head_graph_oc_bs` |
| `24-31` | 羨→qar [*s-N-qar-s {*s-N-[q]a[r]-s}] (0207a) | `qar` | `head_graph_oc_bs` |
| `24-32` | 連→ren [*k‧ren] (None); 僆→ran [*k.ran-s {*k.r[a]n-s}] (213) | `ran` | `head_graph_bs_head` |
| `24-33` | 聯→ron [*k‧ron] (None) | `ron` | `head_graph_oc_shengfu` |
| `24-34` | 輦→ran [*ranʔ {*[r][a][n]ʔ}] (0215a) | `ran` | `head_graph_oc_bs` |
| `24-35` | 嘆→nar [*n̥ˤar-s] (0152a) | `nar` | `head_graph_oc_bs` |
| `24-36` | 然→nan [*nan {*[n]a[n]}] (0217a) | `nan` | `head_graph_oc_bs` |
| `24-37` | 㞋→ten [*trenʔ] (None) | `ten` | `same_character_series_root` |
| `24-38` | 扇→lan [*l̥an-s {*[l̥][a][n]-s}] (0212a) | `lan` | `head_graph_oc_bs` |
| `24-39` | 贊→tsar [*tsˤarʔ-s] (0153a) | `tsar` | `head_graph_oc_bs` |
| `24-40` | 𣦼→san [*za̠n] (None) | `san` | `head_graph_oc_shengfu` |
| `24-41` | 戔→tsan [*dzˤan {*[dz]ˤa[n]}] (0155a) | `tsan` | `head_graph_oc_bs` |
| `24-42` | 𠨧→tsar [*tsʰar {*[tsʰ]ar}] (0206a) | `tsar` | `head_graph_oc_bs` |
| `24-43` | 孱→san [*sɡra̠n] (None) | `san` | `head_graph_oc_shengfu` |
| `24-44` | 散→san [*m-sˤanʔ-s {*m-sˤa[n]ʔ-s}] (0156a) | `san` | `head_graph_oc_bs` |
| `24-45` | 山→ṅar [*s-ŋrar] (0193a) | `ṅar` | `head_graph_oc_bs` |
| `24-46` | 產→ṅar [*s-ŋrarʔ] (0194a) | `ṅar` | `head_graph_oc_bs` |
| `24-47` | 半→pan [*pˤan-s] (0181a) | `pan` | `head_graph_oc_bs` |
| `24-48` | 般→pan [*ba̠n] (None); 般→pat [*pa̠d] (None) | `pan` | `head_graph_mc_coda` |
| `24-49` | 反→pan [*bˤranʔ {*[b]ˤranʔ}] (0262a) | `pan` | `head_graph_oc_bs` |
| `24-50` | 棥→pan [*ban] (None) | `pan` | `head_graph_oc_shengfu` |
| `24-51` | 煩→pan [*ban {*[b]a[n]}] (0264a) | `pan` | `head_graph_oc_bs` |
| `24-52` | 緐→pan [*ban] (None) | `pan` | `head_graph_oc_shengfu` |
| `24-53` | 班→pan [*pʳa̠n] (None) | `pan` | `head_graph_oc_shengfu` |
| `24-54` | 釆→per [*bˤrer-s {*[b]ˤre[r]-s}] (0195a) | `per` | `head_graph_oc_bs` |
| `24-55` | 弁→pon [*C.bron-s {*C.[b]ro[n]-s}] (0220a) | `pon` | `head_graph_oc_bs` |
| `24-56` | 曼→man [*mˤan-s {*mˤa[n]-s}] (0266a) | `man` | `head_graph_oc_bs` |
| `24-57` | 㒼→man [*ma̠n] (None) | `man` | `head_graph_oc_shengfu` |
| `24-58` | 免→mor [*mrorʔ {*mr[o][r]ʔ}] (0222a) | `mor` | `head_graph_oc_bs` |
| `25-01` | 官→kuan [*kʷˤan {*kʷˤa[n]}] (0157a) | `kuan` | `head_graph_oc_bs` |
| `25-02` | 雚→quar [*C.qʷˤar-s] (0158a) | `quar` | `head_graph_oc_bs` |
| `25-03` | 貫→kon [*kˤon {*kˤo[n]}] (0159a) | `kon` | `head_graph_oc_bs` |
| `25-05` | 盥→kon [*ko̠nʔ] (None) | `kon` | `head_graph_oc_shengfu` |
| `25-06` | 款→kon [*kʰo̠nʔ] (None) | `kon` | `head_graph_oc_shengfu` |
| `25-07` | 萈→ɡon [*ɡo̠n] (None) | `ɡon` | `head_graph_oc_shengfu` |
| `25-08` | 萑→ku [*klʲul] (None); 萑→tur [*tur] (None) | `ku` | `head_graph_shengfu_head` |
| `25-09` | 丱→kon [*kʳo̠ns] (None) | `kon` | `head_graph_oc_shengfu` |
| `25-10` | 宦→kuan [*gʷˤran-s {*[g]ʷˤr[a][n]-s}] (0188a) | `kuan` | `head_graph_oc_bs` |
| `25-11` | 卷→kuen [*gʷren {*[g]ʷren}] (0226a); 卷→kor [*krorʔ {*[k]rorʔ}] (0226a) | `kuen` | `head_graph_mc_coda` |
| `25-12` | 亘→quar [*ɢʷˤar {*[ɢ]ʷˤar}] (0164a) | `quar` | `head_graph_oc_bs` |
| `25-13` | 奐→qon [*qʰo̠ns] (None) | `qon` | `head_graph_oc_shengfu` |
| `25-14` | 爰→quan [*ɢʷan {*ɢʷa[n]}] (0255a) | `quan` | `head_graph_oc_bs` |
| `25-15` | 袁→quan [*ɢʷan {*[ɢ]ʷa[n]}] (0256a) | `quan` | `head_graph_oc_bs` |
| `25-16` | 丸→quar [*ɢʷˤar {*[ɢ]ʷˤar}] (0163a) | `quar` | `head_graph_oc_bs` |
| `25-17` | 夗→ʔor [*ʔorʔ {*ʔo[r]ʔ}] (0260a) | `ʔor` | `head_graph_oc_bs` |
| `25-18` | 冤→qon [*qon] (None) | `qon` | `head_graph_oc_shengfu` |
| `25-19` | 元→ṅon [*ŋon {*[ŋ]o[n]} (< nasal + uvular)] (0257a) | `ṅon` | `head_graph_oc_bs` |
| `25-20` | 原→ṅar [*ŋʷar {*[ŋ]ʷar} (< uvular)] (0258a) | `ṅar` | `head_graph_oc_bs` |
| `25-21` | 邍→ṅar [*ŋʷar {*[ŋ]ʷar} (< uvular)] (None) | `ṅar` | `same_character_series_root` |
| `25-22` | 斷→ton [*N-tˤonʔ {*N-tˤo[n]ʔ}] (0170a) | `ton` | `head_graph_oc_bs` |
| `25-23` | 段→ton [*tˤon-s {*tˤo[n]-s}] (0172a) | `ton` | `head_graph_oc_bs` |
| `25-24` | 耑→tor [*tˤor] (0168a) | `tor` | `head_graph_oc_bs` |
| `25-25` | 專→ton [*ton] (0231a) | `ton` | `head_graph_oc_bs` |
| `25-26` | 穿→ton [*tʰon {*tʰo[n]}] (0232a) | `ton` | `head_graph_oc_bs` |
| `25-27` | 舛→ton [*tʰonʔ {*[tʰ]o[n]ʔ}] (0233a) | `ton` | `head_graph_oc_bs` |
| `25-28` | 彖→lon [*l̥ˤon-s {*l̥ˤo[n]-s}] (0171a) | `lon` | `head_graph_oc_bs` |
| `25-29` | 㕣→lon [*ɡ‧lonʔ] (None) | `lon` | `head_graph_oc_shengfu` |
| `25-30` | 鳶→qon [*ɢon] (None) | `qon` | `head_graph_oc_shengfu` |
| `25-31` | 䜌→ron [*mə.rˤon {*mə.rˤo[n]}] (0178a) | `ron` | `head_graph_oc_bs` |
| `25-32` | 卵→ror [*k.rˤorʔ] (0179a) | `ror` | `head_graph_oc_bs` |
| `25-33` | 𤔔→ron [*ɡ‧ro̠ns] (None) | `ron` | `head_graph_oc_shengfu` |
| `25-34` | 奻→ron [*rno̠n] (None) | `ron` | `head_graph_oc_shengfu` |
| `25-35` | 耎→non [*nʲonʔ] (None); 耎→so [*sno] (None) | `non` | `head_graph_mc_coda` |
| `25-36` | 竄→tson [*tsʰˤon-s {*[tsʰ]ˤo[n]-s}] (0176a) | `tson` | `head_graph_oc_bs` |
| `25-37` | 爨→son [*sʰo̠ns] (None) | `son` | `head_graph_oc_shengfu` |
| `25-38` | 全→tson [*dzon {*[dz]o[n]}] (0234a) | `tson` | `head_graph_oc_bs` |
| `25-39` | 雋→son [*sɡonʔ] (None) | `son` | `head_graph_oc_shengfu` |
| `25-40` | 泉→qua [*s-N-ɢʷar I!] (0237a) | `qua` | `head_graph_oc_bs` |
| `25-41` | 縓→ṅon [*sŋ̊on] (None); 縓→ṅar [*ŋʷar {*[ŋ]ʷar} (< uvular)] (None) | `ṅon` | `head_graph_mc_coda` |
| `25-42` | 筭→son [*skˡo̠ns] (None) | `son` | `head_graph_oc_shengfu` |
| `25-43` | 算→son [*skˡo̠nʔ] (None) | `son` | `head_graph_oc_shengfu` |
| `25-44` | 祘→son [*skˡo̠ns] (None) | `son` | `head_graph_oc_shengfu` |
| `26-01` | 笄→ker [*kˤer {*[k]ˤe[r]} (dial. *-r > *-j)] (0586a) | `ker` | `head_graph_oc_bs` |
| `26-02` | 卟→ki [*ki̠] (None) | `ki` | `head_graph_oc_shengfu` |
| `26-03` | 計→kiy [*kˤij-s] (1241a) | `kiy` | `head_graph_oc_bs` |
| `26-04` | 启→kiy [*kʰˤijʔ] (0588a) | `kiy` | `head_graph_oc_bs` |
| `26-05` | 計→kiy [*kˤij-s] (1241a) | `kiy` | `head_graph_oc_bs` |
| `26-06` | 旨→kiy [*kijʔ] (0552a) | `kiy` | `head_graph_oc_bs` |
| `26-07` | 示→kiy [*s-gijʔ-s] (0553a) | `kiy` | `head_graph_oc_bs` |
| `26-08` | 几→ky [*krəjʔ] (0602a) | `ky` | `head_graph_oc_bs` |
| `26-09` | 皆→kiy [*kˤrij] (0599a) | `kiy` | `head_graph_oc_bs` |
| `26-10` | 癸→kuiy [*kʷijʔ] (0605a) | `kuiy` | `head_graph_oc_bs` |
| `26-11` | 醯→qe [*qʰe̠] (None) | `qe` | `head_graph_oc_shengfu` |
| `26-12` | 医→qi [*qi̠s] (None) | `qi` | `head_graph_oc_shengfu` |
| `26-13` | 伊→ʔiy [*ʔij] (0604a) | `ʔiy` | `head_graph_oc_bs` |
| `26-14` | 氐→tiy [*tˤij] (0590a) | `tiy` | `head_graph_oc_bs` |
| `26-15` | 弟→ly [*lˤəjʔ] (0591a) | `ly` | `head_graph_oc_bs` |
| `26-16` | 屖→ly [*s-lˤəj] (0595a) | `ly` | `head_graph_oc_bs` |
| `26-17` | 夷→ly [*ləj] (0551a) | `ly` | `head_graph_oc_bs` |
| `26-18` | 彝→ly [*ləj {*[l][ə]j}] (1237c); 彝→li [*li] (None) | `ly` | `head_graph_bs_head` |
| `26-19` | 矢→liy [*l̥ijʔ {*l̥i[j]ʔ}] (0560a) | `liy` | `head_graph_oc_bs` |
| `26-20` | 尸→ly [*l̥əj {*l̥ə[j]}] (0561a) | `ly` | `head_graph_oc_bs` |
| `26-21` | 兕→qiy [*s.ɢijʔ {*[s.ɢ]ijʔ}] (0556a) | `qiy` | `head_graph_oc_bs` |
| `26-22` | 履→riy [*rijʔ {*[r]ijʔ}] (0562a) | `riy` | `head_graph_oc_bs` |
| `26-23` | 豊→ri [*ri̠ʔ] (None) | `ri` | `head_graph_oc_shengfu` |
| `26-24` | 利→rit [*C.rit-s {*C.ri[t]-s}] (0519a) | `rit` | `head_graph_oc_bs` |
| `26-25` | 尼→nyr [*nˤər-s] (0563a); 尼→ny [*nrəj {*nr[əj]}] (0563a) | `ni` | `packet_shengfu_majority` |
| `26-26` | 妻→tsy [*tsʰˤəj {*[tsʰ]ˤəj}] (0592a) | `tsy` | `head_graph_oc_bs` |
| `26-27` | 齊→tsy [*dzˤəj {*[dz]ˤəj}] (0593a) | `tsy` | `head_graph_oc_bs` |
| `26-28` | 𠂔→sy [*srɯʔ] (0554a) | `sy` | `head_graph_supplement` |
| `26-29` | 次→niy [*s-n̥ij-s {*[s-n̥]i[j]-s}] (0555a) | `niy` | `head_graph_oc_bs` |
| `26-30` | 自→tsit [*N-tsit-s {*N-tsi[t]-s}] (1237m); 自→pit [*S.bit-s {*S.[b]i[t]-s}] (1237m); 自→sit [*sblids] (None) | `tsit` | `head_graph_mc_onset` |
| `26-31` | 西→nyr [*s-nˤər] (0594a) | `nyr` | `head_graph_oc_bs` |
| `26-32` | 屖→ly [*s-lˤəj] (0595a) | `ly` | `head_graph_oc_bs` |
| `26-33` | 犀→ly [*s.lˤəj] (0596a) | `ly` | `head_graph_oc_bs` |
| `26-34` | 厶→si [*sˡil] (None) | `si` | `head_graph_oc_shengfu` |
| `26-35` | 死→siy [*sijʔ] (0558a) | `siy` | `head_graph_oc_bs` |
| `26-36` | 師→siy [*srij] (0559a) | `siy` | `head_graph_oc_bs` |
| `26-37` | 匕→piy [*pijʔ] (0565a) | `piy` | `head_graph_oc_bs` |
| `26-38` | 匕→piy [*pijʔ] (0566a) | `piy` | `head_graph_oc_bs` |
| `26-39` | 米→miy [*C.mˤijʔ] (0598a) | `miy` | `head_graph_oc_bs` |
| `27-01` | 開→ky [*kʰˤəj {*[k]ʰˤəj}] (0541a); 開→y [*Nə-kʰˤəj {*Nə-[k]ʰˤə[j]}] (0541a) | `ky` | `head_graph_supported_root` |
| `27-02` | 豈→qy [*C.qʰəjʔ] (0548a) | `qy` | `head_graph_oc_bs` |
| `27-03` | 冀→kyk [*krək-s {*[k]rə[k]-s} (dial. > *[k]rət-s ?)] (0603a) | `kyk` | `head_graph_oc_bs` |
| `27-04` | 幾→ky [*kəj] (0547a) | `ky` | `head_graph_oc_bs` |
| `27-05` | 衣→ʔy [*ʔəj {*ʔ(r)əj}] (0550a) | `ʔy` | `head_graph_oc_bs` |
| `27-06` | 希→qy [*qʰəj] (0549a) | `qy` | `head_graph_oc_bs` |
| `27-07` | 黹→ti [*tiʔ] (None) | `ti` | `head_graph_oc_shengfu` |
| `27-08` | 非→py [*pəj] (0579a) | `py` | `head_graph_oc_bs` |
| `27-09` | 飛→pyr [*Cə.pər {*Cə.pə[r]} (dial. *-r > *-j)] (0580a) | `pyr` | `head_graph_oc_bs` |
| `27-10` | 妃→py [*pʰəj] (0581a) | `py` | `head_graph_oc_bs` |
| `27-11` | 肥→pyr [*bər {*[b][ə]r}] (0582a) | `pyr` | `head_graph_oc_bs` |
| `27-12` | 火→quy [*qʷʰˤəjʔ {*[qʷʰ]ˤəjʔ}] (0353a) | `quy` | `head_graph_oc_bs` |
| `27-13` | 枚→my [*mɯ̠l] (None) | `my` | `head_graph_oc_shengfu` |
| `27-14` | 眉→myr [*mrər] (0567a) | `myr` | `head_graph_oc_bs` |
| `27-15` | 亹→myr [*mərʔ] (0585a) | `myr` | `head_graph_oc_bs` |
| `27-16` | 美→my [*mrəjʔ {*[m]rəjʔ}] (0568a) | `my` | `head_graph_oc_bs` |
| `27-17` | 尾→my [*məjʔ {*[m]əjʔ}] (0583a) | `my` | `head_graph_oc_bs` |
| `27-18` | 𢼸→my [*mɯl] (None) | `my` | `head_graph_oc_shengfu` |
| `27-19` | 虺→ruy [*r̥ujʔ {*[r̥]u[j]ʔ} (dial. *r̥- > x-)] (0572a) | `ruy` | `head_graph_oc_bs` |
| `27-20` | 蟲→luṅ [*C.lruŋ] (1009a) | `luṅ` | `head_graph_oc_bs` |
| `28-01` | 鬼→ʔuy [*k-ʔujʔ] (0569a) | `ʔuy` | `head_graph_oc_bs` |
| `28-02` | 歸→kuy [*kʷəj {*[k]ʷəj}] (0570a) | `kuy` | `head_graph_oc_bs` |
| `28-03` | 夔→ɡu [*ɡʳul] (None) | `ɡu` | `head_graph_oc_shengfu` |
| `28-04` | 回→quy [*ɢʷˤəj {*[ɢ]ʷˤəj}] (0542a) | `quy` | `head_graph_oc_bs` |
| `28-05` | 韋→quy [*ɢʷəj {*[ɢ]ʷə[j]}] (0571a); 衛→quat [*ɢʷat-s {*[ɢ]ʷ(r)a[t]-s}] (0342a) | `quy` | `merged_packet_primary_head` |
| `28-06` | 褱→ɡu [*ɡʳu̠l] (None) | `ɡu` | `head_graph_oc_shengfu` |
| `28-07` | 乖→kuy [*kʷˤrəj] (1240h); 乖→kui [*kʷʳi̠l] (None) | `kuy` | `head_graph_bs_head` |
| `28-08` | 卉→mu [*m̥ʰulʔ] (None) | `mu` | `head_graph_oc_shengfu` |
| `28-09` | 畏→ʔuy [*ʔuj-s] (0573a) | `ʔuy` | `head_graph_oc_bs` |
| `28-10` | 威→ʔuy [*ʔuj] (0574a) | `ʔuy` | `head_graph_oc_bs` |
| `28-11` | 隹→tur [*tur] (0575a) | `tur` | `head_graph_oc_bs` |
| `28-12` | 𠂤→tuy [*C.tˤuj] (0543a) | `tuy` | `head_graph_oc_bs` |
| `28-13` | 隤→ruy [*N-rˤuj] (0544a) | `ruy` | `head_graph_oc_bs` |
| `28-14` | 水→tay [*s.turʔ (E dialect *-r > *-j)] (0576a) | `tay` | `head_graph_oc_bs` |
| `28-15` | 畾→ru [*ru̠l] (None); 螺→roy [*k.rˤoj] (577); 螺→ro [*ro̠l] (None); 螺→ruy [*rujʔ {*[r]ujʔ}] (None) | `ruy` | `packet_bs_majority` |
| `28-16` | 磊→ruy [*rujʔ {*[r]ujʔ}] (None) | `ruy` | `same_character_series_root` |
| `28-17` | 耒→ru [*rulʔ] (None) | `ru` | `head_graph_oc_shengfu` |
| `28-18` | 蕤→nu [*nʲul] (None) | `nu` | `head_graph_oc_shengfu` |
| `28-19` | 夊→su [*sˡul] (None) | `su` | `head_graph_oc_shengfu` |
| `28-20` | 罪→tsuy [*Cə.dzˤujʔ] (0513a) | `tsuy` | `head_graph_oc_bs` |
| `29-01` | 吉→kit [*kid] (None) | `kit` | `head_graph_oc_shengfu` |
| `29-02` | 鞂→kit [*kʳi̠d] (None); 鞂→kyk [*kˤrək] (None) | `kit` | `head_graph_mc_coda` |
| `29-03` | 棄→kit [*kʰit-s {*[kʰ]i[t]-s}] (0535a) | `kit` | `head_graph_oc_bs` |
| `29-04` | 屆→kit [*kˤrit-s {*kˤr[i][t]-s}] (0510b); 屆→kyt [*kʳɯ̠ds] (None) | `kit` | `head_graph_bs_head` |
| `29-06` | 穴→quit [*ɢʷˤit {*[ɢ]ʷˤi[t]}] (0409a) | `quit` | `head_graph_oc_bs` |
| `29-07` | 血→mik [*m̥ˤik] (0410a) | `mik` | `head_graph_oc_bs` |
| `29-08` | 惠→quiy [*ɢʷˤij-s {*[ɢ]ʷˤi[j]-s}] (0533a) | `quiy` | `head_graph_oc_bs` |
| `29-09` | 㒸→sut [*sɡˡuds] (None) | `sut` | `head_graph_oc_shengfu` |
| `29-10` | 彗→quet [*s-ɢʷet-s {*s-[ɢ]ʷe[t]-s}] (0527a) | `quet` | `head_graph_oc_bs` |
| `29-11` | 矞→qut [*ɢud] (None) | `qut` | `head_graph_oc_shengfu` |
| `29-12` | 一→ʔit [*ʔit {*ʔi[t]}] (0394a) | `ʔit` | `head_graph_oc_bs` |
| `29-13` | 壹→ʔit [*ʔit {*[ʔ]i[t]}] (0395a) | `ʔit` | `head_graph_oc_bs` |
| `29-14` | 替→nit [*n̥ˤit-s {*n̥ˤi[t]-s}] (1241j); 替→lit [*l̥i̠ds] (None) | `nit` | `head_graph_bs_head` |
| `29-15` | 至→tit [*tit-s {*ti[t]-s}] (0413a) | `tit` | `head_graph_oc_bs` |
| `29-16` | 疐→lit [*t-lrit-s {*[t-l]ri[t]-s}] (0415a) | `lit` | `head_graph_oc_bs` |
| `29-17` | 失→lit [*l̥it {*l̥i[t]}] (0402a) | `lit` | `head_graph_oc_bs` |
| `29-18` | 實→lit [*mə.lit {*mə.li[t]}] (0398a) | `lit` | `head_graph_oc_bs` |
| `29-19` | 逸→lit [*lit {*[l]i[t]}] (0396a) | `lit` | `head_graph_oc_bs` |
| `29-20` | 溢→qik [*N-qik {*N-q(r)ik}] (1257a) | `qik` | `head_graph_oc_bs` |
| `29-21` | 佾→rit [*N.rit {*[N.]r[i]t}] (0397a) | `rit` | `head_graph_oc_bs` |
| `29-22` | 欥→qut [*ɢud] (None); 欥→quat [*ɢʷat {*[ɢ]ʷat}] (None) | `qut` | `head_graph_shengfu_head` |
| `29-23` | 栗→rit [*rit {*[r]i[t]}] (0403a) | `rit` | `head_graph_oc_bs` |
| `29-24` | 盭→ryt [*rɯ̠d] (None) | `ryt` | `head_graph_oc_shengfu` |
| `29-25` | 戾→ret [*rˤet-s {*[r]ˤe[t]-s}] (0532a) | `ret` | `head_graph_oc_bs` |
| `29-26` | 日→nit [*C.nit {*C.ni[t]}] (0404a) | `nit` | `head_graph_oc_bs` |
| `29-27` | 臸→tit [*tit-s {*ti[t]-s}] (0414a) | `tit` | `head_graph_supplement` |
| `29-28` | 二→niy [*nij-s {*ni[j]-s}] (0564a) | `niy` | `head_graph_oc_bs` |
| `29-29` | 疾→tsit [*dzit {*[dz]i[t]}] (0494a) | `tsit` | `head_graph_oc_bs` |
| `29-30` | 卽→tsik [*tsik {*[ts]ik}] (None); 即→syk [*sɯɡ] (None); 卪→sik [*si̠ɡ] (None) | `tsik` | `head_graph_supported_root` |
| `29-31` | 七→tsit [*tsʰit {*[tsʰ]i[t]}] (0400a) | `tsit` | `head_graph_oc_bs` |
| `29-32` | 桼→tsit [*tsʰit {*[tsʰ]i[t]}] (0401a) | `tsit` | `head_graph_oc_bs` |
| `29-33` | 溢→qik [*N-qik {*N-q(r)ik}] (1257a) | `qik` | `head_graph_oc_bs` |
| `29-34` | 四→liy [*s.lij-s {*s.li[j]-s}] (0518a) | `liy` | `head_graph_oc_bs` |
| `29-35` | 蝨→sit [*srit {*sri[t]}] (0506a) | `sit` | `head_graph_oc_bs` |
| `29-36` | 瑟→mit [*s.mrit {*[s.m]ri[t]}] (0411a) | `mit` | `head_graph_oc_bs` |
| `29-37` | 閉→pit [*pˤit-s {*pˤi[t]-s}] (0412a) | `pit` | `head_graph_oc_bs` |
| `29-38` | 匹→pit [*pʰit {*pʰi[t]}] (0408a) | `pit` | `head_graph_oc_bs` |
| `29-39` | 畀→pik [*pik-s {*pi[k]-s}] (0521a) | `pik` | `head_graph_oc_bs` |
| `29-41` | 必→pit [*pit {*pi[t]}] (0405a) | `pit` | `head_graph_oc_bs` |
| `29-42` | 畢→pit [*pit {*pi[t]}] (0407a) | `pit` | `head_graph_oc_bs` |
| `29-43` | 觱→qyt [*ɢpɯd] (None); 觱→qik [*N-qik {*N-q(r)ik}] (None); 觱→pit [*pid] (None) | `qik` | `head_graph_supported_root` |
| `29-44` | 弜→pit [*pit {*pi[t]}] (0406a) | `pit` | `head_graph_supplement` |
| `30-01` | 气→qyp [*C.qʰəp-s] (0517a) | `qyp` | `head_graph_oc_bs` |
| `30-02` | 旡→kyt [*kɯds] (None) | `kyt` | `head_graph_oc_shengfu` |
| `30-03` | 器→kyt [*kʰrət-s {*[kʰ]r[ə][t]-s}] (0536a) | `kyt` | `head_graph_oc_bs` |
| `30-04` | 洎→ɡqup [*ɡʳqubs] (None); 洎→ryp [*m-rˤəp] (None); 洎→kup [*kʳubs] (None) | `ryp` | `head_graph_supported_root` |
| `30-05` | 戛→kik [*kˤrik {*kˤri[k]}] (0504a) | `kik` | `head_graph_oc_bs` |
| `30-06` | 愛→qyp [*qˤəp-s {*[q]ˤəp-s}] (0508a) | `qyp` | `head_graph_oc_bs` |
| `30-07` | 乙→qyt [*qrət] (0505a) | `qyt` | `head_graph_oc_bs` |
| `30-08` | 劓→ṅit [*ŋʳids] (None) | `ṅit` | `head_graph_oc_shengfu` |
| `30-09` | 豙→ṅyt [*ŋət-s {*[ŋ]ə[t]-s}] (0516a) | `ṅyt` | `head_graph_oc_bs` |
| `30-10` | 質→tip [*t<r>ip-s {*[t]<r>ip-s}] (0493a); 質→lit [*t-lit] (0493a) | `lit` | `head_graph_mc_coda` |
| `30-11` | 隶→ryp [*m.rˤəp-s] (0509a) | `ryp` | `head_graph_oc_bs` |
| `30-12` | 配→pyt [*pʰɯ̠ds] (None); 配→pyr [*bər {*[b][ə]r}] (None) | `pyt` | `head_graph_shengfu_head` |
| `30-13` | 孛→put [*bˤut-s {*[b]ˤ[u]t-s}] (0491a) | `put` | `head_graph_oc_bs` |
| `30-14` | 弗→put [*put] (0500a) | `put` | `head_graph_oc_bs` |
| `30-16` | 巿→put [*pud] (None) | `put` | `head_graph_oc_shengfu` |
| `30-17` | 胐→py [*pʰəjʔ {*pʰ[ə]jʔ}] (None); 朏→kut [*kʰu̠d] (None) | `py` | `head_graph_supported_root` |
| `30-18` | 𠬛→mut [*mu̠d] (None) | `mut` | `head_graph_oc_shengfu` |
| `30-19` | 勿→mut [*mut] (0503a) | `mut` | `head_graph_oc_bs` |
| `30-20` | 勿→mut [*mut] (0503a) | `mut` | `head_graph_oc_bs` |
| `30-21` | 未→myt [*mət-s {*m[ə]t-s}] (0531a) | `myt` | `head_graph_oc_bs` |
| `30-22` | 鬽→myt [*mət-s {*m[ə]t-s}] (None) | `myt` | `same_character_series_root` |
| `31-01` | 骨→kut [*kˤut] (0486a) | `kut` | `head_graph_oc_bs` |
| `31-02` | 㬰→kuy [*gruj-s {*[g]ruj-s}] (0540a) | `kuy` | `head_graph_oc_bs` |
| `31-03` | 凷→kyt [*kʰɯ̠ds] (None) | `kyt` | `head_graph_oc_shengfu` |
| `31-04` | 㕟→kut [*kʰʳu̠ds] (None) | `kut` | `head_graph_oc_shengfu` |
| `31-05` | 胃→quyt [*ɢʷət-s {*[ɢ]ʷə[t]-s}] (0523a) | `quyt` | `head_graph_oc_bs` |
| `31-06` | 彙→qut [*ɢʳuds] (None); 彙→quyt [*ɢʷət-s {*[ɢ]ʷə[t]-s}] (None) | `qut` | `head_graph_shengfu_head` |
| `31-07` | 位→quyp [*ɢʷrəp-s {*[ɢ]ʷrəp-s} < *ɢʷəʔ-rəp-s ?] (0539a) | `quyp` | `head_graph_oc_bs` |
| `31-08` | 𩰪→qut [*qut] (None) | `qut` | `same_character_series_root` |
| `31-09` | 㷉→ʔut [*ʔut-s] (0525a) | `ʔut` | `head_graph_oc_bs` |
| `31-10` | 兀→ṅut [*ŋu̠d] (None); 兀→ṅon [*ŋon {*[ŋ]o[n]} (< nasal + uvular)] (None) | `ṅut` | `head_graph_mc_coda` |
| `31-11` | 𠫓→tut [*tʰu̠d] (None) | `tut` | `head_graph_oc_shengfu` |
| `31-12` | 突→tut [*m-tʰˤut] (0489a) | `tut` | `head_graph_oc_bs` |
| `31-14` | 對→typ [*tˤəp-s {*[t]ˤ[ə]p-s}] (0511a) | `typ` | `head_graph_oc_bs` |
| `31-15` | 退→nyp [*n̥ˤəp-s {*n̥ˤ[ə]p-s}] (0512a) | `nyp` | `head_graph_oc_bs` |
| `31-16` | 出→kut [*t.kʰut-s] (0496a) | `kut` | `head_graph_oc_bs` |
| `31-17` | 朮→lut [*mə.lut ~ *m.lut] (0497a) | `lut` | `head_graph_oc_bs` |
| `31-18` | 聿→rut [*N.rut {*[N.]rut}] (0502a) | `rut` | `head_graph_oc_bs` |
| `31-19` | 類→rut [*rut-s {*[r]u[t]-s}] (0529a) | `rut` | `head_graph_oc_bs` |
| `31-20` | 卒→tsut [*tsut {*[ts]ut}] (0490a) | `tsut` | `head_graph_oc_bs` |
| `31-21` | 祟→lut [*s-lut-s] (0528a) | `lut` | `head_graph_oc_bs` |
| `31-22` | 戌→mit [*s.mit {*s.mi[t]}] (1257h); 戌→sit [*smid] (None) | `mit` | `head_graph_bs_head` |
| `31-23` | 率→rut [*rut {*[r]ut}] (0498a) | `rut` | `head_graph_oc_bs` |
| `31-24` | 帥→rut [*s-rut-s] (0499a) | `rut` | `head_graph_oc_bs` |
| `32-01` | 臤→kiṅ [*kʰʳi̠ŋ] (None); 臤→kin [*gin {*[g]i[n]}] (None); 臣→ɡʲiṅ [*ɡʲiŋ] (None) | `kin` | `head_graph_supported_root` |
| `32-02` | 繭→ken [*ke̠nʔ] (None) | `ken` | `head_graph_oc_shengfu` |
| `32-03` | 矜→kiṅ [*griŋ] (0369a); 矜→i [*k.riŋ (dial. > k-, *-iŋ > -ing)] (0369a) | `kiṅ` | `head_graph_mc_coda` |
| `32-04` | 犬→kuen [*kʷʰˤenʔ {*[k]ʷʰˤ[e][n]ʔ}] (0479a) | `kuen` | `head_graph_oc_bs` |
| `32-05` | 玄→quin [*ɢʷˤin {*[ɢ]ʷˤi[n]}] (0366a) | `quin` | `head_graph_oc_bs` |
| `32-06` | 贙→ɡin [*ɡʷi̠nʔ] (None) | `ɡin` | `head_graph_oc_shengfu` |
| `32-07` | 勻→quiṅ [*ɢʷiŋ] (None) | `quiṅ` | `head_graph_oc_shengfu` |
| `32-08` | 旬→quin [*s-ɢʷin {*s-[ɢ]ʷi[n]}] (0392a) | `quin` | `head_graph_oc_bs` |
| `32-09` | 因→ʔin [*ʔin {*ʔi[n]}] (0370a) | `ʔin` | `head_graph_oc_bs` |
| `32-10` | 垔→qin [*qin {*[q]i[n]}] (0483a) | `qin` | `head_graph_oc_bs` |
| `32-11` | 覦→lo [*ɡ‧lo] (None) | `lo` | `head_graph_oc_shengfu` |
| `32-12` | 淵→ʔin [*ʔʷˤin {*[ʔ]ʷˤi[n]}] (0367a) | `ʔin` | `head_graph_oc_bs` |
| `32-13` | 嚚→ṅiṅ [*ŋɡʳiŋ] (None); 嚚→kin [*gin {*[g]i[n]}] (None) | `kin` | `head_graph_mc_coda` |
| `32-14` | 憖→ṅyṅ [*ŋɡʳɯŋs] (None) | `ṅyṅ` | `head_graph_oc_shengfu` |
| `32-15` | 天→lin [*l̥ˤin {*l̥ˤi[n]}] (0361a) | `lin` | `head_graph_oc_bs` |
| `32-16` | 真→tin [*tin {*ti[n]}] (0375a) | `tin` | `head_graph_oc_bs` |
| `32-17` | 眘→tin [*tin {*ti[n]}] (None) | `tin` | `same_character_series_root` |
| `32-18` | 陳→lin [*lrin {*lri[n]}] (0373a) | `lin` | `head_graph_oc_bs` |
| `32-19` | 田→liṅ [*lˤiŋ] (0362a) | `liṅ` | `head_graph_oc_bs` |
| `32-20` | 引→lin [*linʔ-s {*li[n]ʔ-s}] (0371a) | `lin` | `head_graph_oc_bs` |
| `32-21` | 絼→lin [*linʔ] (None) | `lin` | `same_character_series_root` |
| `32-22` | 申→lin [*l̥in {*l̥i[n]}] (0385a) | `lin` | `head_graph_oc_bs` |
| `32-23` | 身→niṅ [*n̥iŋ {*n̥i[ŋ]}] (0386a) | `niṅ` | `head_graph_oc_bs` |
| `32-24` | 舜→qun [*qʰuns] (None); 舜→lon [*ɡ‧lonʔ] (None) | `qun` | `head_graph_shengfu_head` |
| `32-25` |  | `` | `` |
| `32-26` | 粦→ryn [*rən {*[r]ə[n]}] (0387a) | `ryn` | `head_graph_oc_bs` |
| `32-27` | 覦→lo [*ɡ‧lo] (None) | `lo` | `head_graph_oc_shengfu` |
| `32-28` | 人→niṅ [*niŋ] (0388a) | `niṅ` | `head_graph_oc_bs` |
| `32-29` | 晉→tsin [*tsin-s {*tsi[n]-s}] (0378a) | `tsin` | `head_graph_oc_bs` |
| `32-30` | 進→tsin [*tsin-s {*[ts][i][n]-s}] (0379a) | `tsin` | `head_graph_oc_bs` |
| `32-31` | 秦→tsin [*dzin {*[dz]i[n]}] (0380a) | `tsin` | `head_graph_oc_bs` |
| `32-32` | 盡→tsin [*Cə.dzinʔ {*Cə.[dz]i[n]ʔ}] (0381a) | `tsin` | `head_graph_oc_bs` |
| `32-33` | 辛→sin [*sin {*[s]i[n]}] (0382a) | `sin` | `head_graph_oc_bs` |
| `32-34` |  | `` | `` |
| `32-35` | 卂→syn [*sˡɯns] (None) | `syn` | `head_graph_oc_shengfu` |
| `32-36` | 信→niṅ [*s-niŋ-s {*s-ni[ŋ]-s}] (0384a) | `niṅ` | `head_graph_oc_bs` |
| `32-37` | 甡→sin [*sʳin] (None) | `sin` | `head_graph_oc_shengfu` |
| `32-38` | 賔→pin [*pin {*pi[n]}] (None); 賓→men [*mbe̠nʔ] (None) | `pin` | `head_graph_supported_root` |
| `32-39` | 頻→pin [*bin {*[b]i[n]}] (0390a) | `pin` | `head_graph_oc_bs` |
| `32-40` | 民→miṅ [*miŋ {*mi[ŋ]}] (0457a) | `miṅ` | `head_graph_oc_bs` |
| `33-01` | 艮→kyn [*kɯ̠ns] (None) | `kyn` | `head_graph_oc_shengfu` |
| `33-02` | 斤→kyr [*kər {*[k]ər}] (0443a) | `kyr` | `head_graph_oc_bs` |
| `33-03` | 筋→kyn [*C.kən {*C.[k]ə[n]}] (0445a) | `kyn` | `head_graph_oc_bs` |
| `33-04` | 𢀷→kyn [*kɯnʔ] (None) | `kyn` | `same_character_series_root` |
| `33-05` | 𡏳→kyr [*kˤrər {*kˤrə[r]}] (0480a) | `kyr` | `head_graph_oc_bs` |
| `33-06` | 巾→kyn [*krən] (0482a) | `kyn` | `head_graph_oc_bs` |
| `33-07` | 釁→myr [*m̥rər-s] (0446a) | `myr` | `head_graph_oc_bs` |
| `33-08` | 衅→myr [*m̥rər-s] (None) | `myr` | `same_character_series_root` |
| `33-09` | 殷→ʔyr [*ʔˤrər {*ʔˤrə[r]}] (0448a) | `ʔyr` | `head_graph_oc_bs` |
| `33-10` | 隱→ʔyr [*ʔərʔ-s {*ʔ(r)ə[r]ʔ-s}] (0449a); 隱→ʔyn [*ʔənʔ {*[ʔ](r)ə[n]ʔ}] (0449a) | `ʔyn` | `head_graph_mc_coda` |
| `33-11` | 典→tyr [*tˤərʔ {*tˤə[r]ʔ}] (0476a) | `tyr` | `head_graph_oc_bs` |
| `33-12` | 𡱂→tyn [*dˤən {*[d]ˤə[n]} (dial. > -won)] (0429a) | `tyn` | `head_graph_oc_bs` |
| `33-13` | 辰→tyr [*dər {*[d]ər}] (0455a) | `tyr` | `head_graph_oc_bs` |
| `33-14` | 憖→ṅyṅ [*ŋɡʳɯŋs] (None) | `ṅyṅ` | `head_graph_oc_shengfu` |
| `33-15` | 㐱→tyn [*tənʔ {*t[ə][n]ʔ}] (0453a) | `tyn` | `head_graph_oc_bs` |
| `33-16` | 疢→tyn [*tʰɯns] (None) | `tyn` | `head_graph_oc_shengfu` |
| `33-17` | 塵→tyn [*drən {*[d]rə[n]}] (0374a) | `tyn` | `head_graph_oc_bs` |
| `33-18` | 胤→qyr [*[ɢ]ər-s {*[ɢ]ə[n]-s}] (0451a) | `qyr` | `head_graph_oc_bs` |
| `33-19` | 寅→qyr [*ɢər {*[ɢ](r)ər}] (0450a) | `qyr` | `head_graph_oc_bs` |
| `33-20` | 刃→nyn [*nʲɯns] (None) | `nyn` | `head_graph_oc_shengfu` |
| `33-22` | 存→tsyn [*dzˤən {*[dz]ˤə[n]}] (0432a) | `tsyn` | `head_graph_oc_bs` |
| `33-23` | 薦→tsyr [*Cə.tsˤər-s {*Cə.[ts]ˤə[r]-s}] (0477a) | `tsyr` | `head_graph_oc_bs` |
| `33-24` | 齔→syn [*sʰrɯnʔ] (None); 齔→tsit [*tsʰit {*[tsʰ]i[t]}] (None); 齔→sin [*sʰrins] (None) | `tsit` | `head_graph_supported_root` |
| `33-25` | 先→syr [*sˤər] (0478a) | `syr` | `head_graph_oc_bs` |
| `33-26` | 畚→pun [*pu̠nʔ] (None); 畚→pon [*C.bron-s {*C.[b]ro[n]-s}] (None) | `pun` | `head_graph_shengfu_head` |
| `33-27` | 本→pyn [*C.pˤənʔ {*C.pˤə[n]ʔ}] (0440a) | `pyn` | `head_graph_oc_bs` |
| `33-28` | 奔→pur [*pˤur] (0438a) | `pur` | `head_graph_oc_bs` |
| `33-29` | 賁→pur [*bur {*[b]ur}] (0437a) | `pur` | `head_graph_oc_bs` |
| `33-30` | 分→pyn [*m-pən-s {*[m]-pə[n]-s}] (0471a) | `pyn` | `head_graph_oc_bs` |
| `33-32` | 糞→pur [*pur-s {*p[u]r-s}] (0472a) | `pur` | `head_graph_oc_bs` |
| `33-33` | 奮→pyn [*pən-s {*p[ə][n]-s}] (0473a) | `pyn` | `head_graph_oc_bs` |
| `33-34` | 焚→pyn [*bɯn] (None) | `pyn` | `head_graph_oc_shengfu` |
| `33-35` | 門→myn [*mˤən {*mˤə[n]}] (0441a) | `myn` | `head_graph_oc_bs` |
| `33-36` | 文→myn [*mən {*mə[n]}] (0475a) | `myn` | `head_graph_oc_bs` |
| `34-01` | 昆→kun [*kˤun {*kˤu[n]}] (0417a) | `kun` | `head_graph_oc_bs` |
| `34-02` | 衮→kun [*kˡu̠nʔ] (None) | `kun` | `same_character_series_root` |
| `34-03` | 鯀→kun [*ku̠nʔ] (None) | `kun` | `head_graph_oc_shengfu` |
| `34-04` | 鰥→kuyn [*kʷˤrən {*[k]ʷˤrə[n]}] (0481a) | `kuyn` | `head_graph_oc_bs` |
| `34-05` | 坤→kun [*kʰˤun {*[kʰ]ˤu[n]}] (0421a) | `kun` | `head_graph_oc_bs` |
| `34-06` | 巛→tun [*tʰun {*[tʰ]u[n]} (MC -jwen is irregular)] (0422a) | `tun` | `head_graph_oc_bs` |
| `34-07` | 髡→kun [*kʰu̠n] (None); 髡→ṅut [*ŋu̠d] (None) | `kun` | `head_graph_mc_coda` |
| `34-08` | 壼→kun [*kʰʳun] (None) | `kun` | `head_graph_oc_shengfu` |
| `34-09` | 困→kun [*kʰˤun-s {*kʰˤu[n]-s}] (0420a) | `kun` | `head_graph_oc_bs` |
| `34-10` | 圂→ɡun [*ɡu̠ns] (None) | `ɡun` | `head_graph_oc_shengfu` |
| `34-11` | 囷→kun [*kʰʳun] (None) | `kun` | `head_graph_oc_shengfu` |
| `34-12` | 君→qur [*C.qur] (0459a) | `qur` | `head_graph_oc_bs` |
| `34-13` | 軍→kuyr [*kʷər {*[k]ʷər}] (0458a) | `kuyr` | `head_graph_oc_bs` |
| `34-14` | 云→quyr [*ɢʷər {*[ɢ]ʷə[r]}] (0460a) | `quyr` | `head_graph_oc_bs` |
| `34-15` | 熏→qun [*qʰun {*qʰu[n]}] (0461a) | `qun` | `head_graph_oc_bs` |
| `34-16` | 𥁕→ʔun [*ʔˤun] (0426a) | `ʔun` | `head_graph_oc_bs` |
| `34-17` | 屯→tun [*dˤun {*[d]ˤun}] (0427a) | `tun` | `head_graph_oc_bs` |
| `34-18` | 𦎧→tun [*dʲun] (None) | `tun` | `same_character_series_root` |
| `34-19` | 春→tun [*tʰun] (0463a) | `tun` | `head_graph_oc_bs` |
| `34-20` | 川→lun [*t.l̥un {*t.l̥u[n]} (MC -jwen is irregular)] (0462a) | `lun` | `head_graph_oc_bs` |
| `34-21` | 盾→lun [*lˤunʔ {*lˤu[n]ʔ}] (0465a) | `lun` | `head_graph_oc_bs` |
| `34-22` | 豚→lun [*lˤun {*lˤu[n]}] (0428a) | `lun` | `head_graph_oc_bs` |
| `34-23` | 允→lur [*lurʔ {*[l]u[r]ʔ}] (0468a) | `lur` | `head_graph_oc_bs` |
| `34-24` | 侖→run [*k‧run] (None) | `run` | `head_graph_oc_shengfu` |
| `34-25` | 閏→nun [*nʲuns] (None) | `nun` | `head_graph_oc_shengfu` |
| `34-26` | 尊→tsun [*tsˤun {*[ts]ˤu[n]}] (0430a) | `tsun` | `head_graph_oc_bs` |
| `34-27` | 寸→tsun [*tsʰˤun-s {*[tsʰ]ˤu[n]-s}] (0431a) | `tsun` | `head_graph_oc_bs` |
| `34-28` | 孫→sun [*sˤun {*[s]ˤu[n]}] (0434a) | `sun` | `head_graph_oc_bs` |
| `34-29` | 飧→sun [*sˡu̠n] (None) | `sun` | `head_graph_oc_shengfu` |
| `34-30` | 巽→sun [*skˡu̠ns] (None) | `sun` | `head_graph_oc_shengfu` |
| `34-31` | 隼→sun [*sqˡunʔ] (None); 隼→tur [*tur] (None) | `sun` | `head_graph_mc_coda` |
| `34-32` | 䜭→ɡon [*ɡʳon] (None); 䜭→qot [*ɢods] (None) | `ɡon` | `head_graph_mc_coda` |
| `35-02` | 甲→kap [*kˤrap {*[k]ˤr[a]p}] (0629a) | `kap` | `head_graph_oc_bs` |
| `35-03` | 夾→kep [*C.kˤ<r>ep] (0630a) | `kep` | `head_graph_oc_bs` |
| `35-04` | 劦→ɡyp [*ɡɯ̠b] (None); 劦→ryp [*ɡ‧rɯ̠b] (None) | `ɡyp` | `head_graph_supported_root` |
| `35-05` | 業→qap [*m-qʰap {*[m-qʰ](r)[a]p}] (0640a); 業→ṅap [*ŋap {*[ŋ](r)[a]p}] (0640a) | `ṅap` | `head_graph_mc_onset` |
| `35-06` | 涉→tep [*dʲeb] (None) | `tep` | `head_graph_oc_shengfu` |
| `35-07` | 𦐇→tap [*tʰa̠b] (None) | `tap` | `head_graph_oc_shengfu` |
| `35-09` | 耴→nep [*t-nrep] (0632a) | `nep` | `head_graph_oc_bs` |
| `35-10` | 枼→lep [*ɡ‧leb] (None); 枼→lap [*l̥ap-s {*l̥[a]p-s}] (None) | `lep` | `head_graph_shengfu_head` |
| `35-11` | 疊→lyp [*lɯ̠b] (None) | `lyp` | `head_graph_oc_shengfu` |
| `35-12` | 巤→rap [*rab] (None) | `rap` | `head_graph_oc_shengfu` |
| `35-13` | 聶→nep [*nrep] (0638a) | `nep` | `head_graph_oc_bs` |
| `35-14` | 苶→mep [*mle̠b] (None); 苶→ne [*nˤeʔ {*nˤe(j)ʔ}] (None); 苶→met [*mlʲed] (None) | `mep` | `head_graph_mc_coda` |
| `35-15` | 妾→tsap [*tsʰap {*[tsʰ]ap}] (0635a) | `tsap` | `head_graph_oc_bs` |
| `35-16` | 疌→sep [*sɡeb] (None) | `sep` | `head_graph_oc_shengfu` |
| `35-17` | 臿→tsop [*tsʰ<r>op {*[tsʰ](ˤ)<r>op}] (0631a) | `tsop` | `head_graph_oc_bs` |
| `35-18` | 扱→syp [*skʰrɯ̠b] (None); 扱→ryp [*m-k-rəp {*[m-k-]rəp}] (None) | `syp` | `head_graph_shengfu_head` |
| `35-19` | 燮→sep [*sˡe̠b] (None) | `sep` | `head_graph_oc_shengfu` |
| `35-20` | 乏→pop [*bop {*[b](r)[o]p}] (0641a) | `pop` | `head_graph_oc_bs` |
| `35-21` | 劫→kap [*kap {*k(r)ap}] (0642h); 劫→kop [*kob] (None); 法→pap [*pkab] (None); 盍→ɡap [*ɡa̠b] (None); 磕→kat [*kʰa̠d] (None); 饁→qap [*ɢʳab] (None) | `kap` | `head_graph_supported_root` |
| `36-01` | 甘→kam [*kˤam {*[k]ˤ[a]m}] (0606a) | `kam` | `head_graph_oc_bs` |
| `36-02` | 敢→kam [*kˤamʔ {*[k]ˤamʔ}] (0607a); 夏→ka [*gˤraʔ {*[g]ˤraʔ} ~ [g]ˤraʔ-s] (0036a); 夏→qa [*ɢˤraʔ {*[ɢ]ˤraʔ}] (0036a) | `kam` | `merged_packet_primary_head` |
| `36-03` | 銜→kam [*Cə.gˤram {*Cə.[g]ˤr[a]m}] (0608a) | `kam` | `head_graph_oc_bs` |
| `36-04` | 欠→kom [*kʰom-s {*[k]ʰ(r)om-s}] (0624a) | `kom` | `head_graph_oc_bs` |
| `36-05` | 監→kam [*kˤram {*[k]ˤram}] (0609a) | `kam` | `head_graph_oc_bs` |
| `36-06` | 僉→som [*skʰlom] (None) | `som` | `head_graph_oc_shengfu` |
| `36-07` | 兼→kem [*kˤem {*[k]ˤem}] (0627a) | `kem` | `head_graph_oc_bs` |
| `36-09` | 弇→kom [*ko̠m] (None); 弇→kop [*m-kˤop] (None); 弇→qom [*qʳomʔ] (None) | `kop` | `head_graph_supported_root` |
| `36-10` | 猒→ʔem [*qem] (None) | `ʔem` | `same_character_series_root` |
| `36-11` | 詹→tam [*tam {*[t]am}] (0619a) | `tam` | `head_graph_oc_bs` |
| `36-12` | 占→tem [*tem] (0618a) | `tem` | `head_graph_oc_bs` |
| `36-13` | 豓→lim [*l̥ˤimʔ-s {*l̥ˤ[i]mʔ-s}] (None) | `lim` | `same_character_series_root` |
| `36-14` | 炎→qam [*ɢʳam] (None) | `qam` | `head_graph_oc_shengfu` |
| `36-16` | 銛→kot [*ko̠d] (None); 銛→sem [*skˡem] (None); 銛→kem [*kʰle̠mʔ] (None) | `kot` | `head_graph_supported_root` |
| `36-17` | 忝→lim [*l̥ˤimʔ-s {*l̥ˤ[i]mʔ-s}] (1247c); 忝→lin [*l̥ˤin {*l̥ˤi[n]}] (None) | `lim` | `head_graph_mc_coda` |
| `36-18` | 冄→nam [*nʲam] (None) | `nam` | `same_character_series_root` |
| `36-19` | 染→nam [*C.namʔ {*C.n[a]mʔ}] (0623a) | `nam` | `head_graph_oc_bs` |
| `36-21` | 㦰→sem [*sem] (None) | `sem` | `head_graph_oc_shengfu` |
| `36-22` | 斬→tsam [*tsramʔ {*[ts]ramʔ}] (0611a) | `tsam` | `head_graph_oc_bs` |
| `36-23` | 毚→sum [*zru̠m] (None); 毚→som [*zro̠m] (None) | `som` | `packet_shengfu_majority` |
| `36-24` | 髟→sam [*sˤram {*s[ˤ]ram}] (1154a) | `sam` | `head_graph_oc_bs` |
| `36-25` | 芟→som [*sʳo̠m] (None) | `som` | `head_graph_oc_shengfu` |
| `36-26` | 凡→pom [*brom {*[b]rom}] (0625a) | `pom` | `head_graph_oc_bs` |
| `36-27` | 犯→pom [*bomʔ] (None) | `pom` | `head_graph_oc_shengfu` |
| `37-01` | 合→kop [*m-kˤop] (0675a); 拾→typ [*dəp {*[d][ə]p}] (0687a) | `kop` | `merged_packet_primary_head` |
| `37-02` | 及→ryp [*m-k-rəp {*[m-k-]rəp}] (0681a) | `ryp` | `head_graph_oc_bs` |
| `37-03` | 十→kyp [*t.gəp {*t.[g]əp}] (0686a) | `kyp` | `head_graph_oc_bs` |
| `37-04` | 燁→qip [*ɢʳib] (None) | `qip` | `head_graph_oc_shengfu` |
| `37-05` | 邑→qyp [*qəp {*q(r)[ə]p}] (0683a) | `qyp` | `head_graph_oc_bs` |
| `37-06` | 答→typ [*tˤəp {*[t]ˤ[ə]p}] (0676a) | `typ` | `head_graph_oc_bs` |
| `37-07` | 馽→tip [*tib] (None) | `tip` | `head_graph_oc_shengfu` |
| `37-08` | 執→tip [*tip {*[t]ip}] (0685a) | `tip` | `head_graph_oc_bs` |
| `37-09` | 沓→ryp [*m-rˤəp] (0677a) | `ryp` | `head_graph_oc_bs` |
| `37-10` | 眔→ryp [*m-rˤəp] (0678a) | `ryp` | `head_graph_oc_bs` |
| `37-11` | 襲→syp [*sə-ləp {*sə-l[ə]p}] (0689a); 襲→lyp [*sə.ləp {*sə.l[ə]p}] (0689a) | `lyp` | `head_graph_supported_root` |
| `37-12` | 習→syp [*sɡˡɯb] (None) | `syp` | `head_graph_oc_shengfu` |
| `37-13` | 濕→qip [*qʰip {*[qʰ]ip}] (0692a) | `qip` | `head_graph_oc_bs` |
| `37-14` | 溼→qip [*s-qʰip {*s-[qʰ][i]p}] (0693a) | `qip` | `head_graph_oc_bs` |
| `37-15` | 立→ryp [*k.rəp] (0694a) | `ryp` | `head_graph_oc_bs` |
| `37-16` | 入→nyp [*nəp {*n[ə]p}] (0695a) | `nyp` | `head_graph_oc_bs` |
| `37-17` | 㚔→keṅ [*gˤreŋʔ {*[g]ˤreŋʔ}] (None) | `keṅ` | `same_character_series_root` |
| `37-18` | 帀→sup [*su̠b] (None) | `sup` | `head_graph_oc_shengfu` |
| `37-19` | 咠→sip [*skib] (None) | `sip` | `head_graph_oc_shengfu` |
| `37-20` | 集→tsyp [*dzəp {*(Cə.)[dz][ə]p}] (0691a) | `tsyp` | `head_graph_oc_bs` |
| `37-21` | 颯→syp [*sqˡɯ̠b] (None); 颯→ryp [*k.rəp] (None) | `syp` | `head_graph_shengfu_head` |
| `38-02` | 圅→ym [*Cə-m-kˤəm {*Cə-m-kˤ[ə]m}] (0643a) | `ym` | `head_graph_oc_bs` |
| `38-03` | 今→kym [*krəm {*[k]r[ə]m}] (0651a) | `kym` | `head_graph_oc_bs` |
| `38-04` | 咸→kym [*gˤrəm {*[g]ˤr[ə]m}] (0671a) | `kym` | `head_graph_oc_bs` |
| `38-05` | 臽→kom [*gˤrom-s {*[g]ˤrom(ʔ)-s}] (0672a) | `kom` | `head_graph_oc_bs` |
| `38-06` | 熊→quym [*C.ɢʷəm {*C.[ɢ]ʷ(r)əm}] (0674a) | `quym` | `head_graph_oc_bs` |
| `38-07` | 音→qym [*qəm {*[q](r)[ə]m}] (0653a) | `qym` | `head_graph_oc_bs` |
| `38-08` | 飲→qym [*qəmʔ-s {*q(r)[ə]mʔ-s}] (0654a) | `qym` | `head_graph_oc_bs` |
| `38-09` | 儑→ṅup [*ŋɡu̠b] (None); 儑→qip [*s-qʰip {*s-[qʰ][i]p}] (None); 儑→ṅum [*ŋɡu̠ms] (None) | `ṅum` | `head_graph_mc_coda` |
| `38-10` | 碞→ṅym [*ŋʳɯ̠m] (None) | `ṅym` | `head_graph_oc_shengfu` |
| `38-11` | 甚→qym [*t.ɢəmʔ-s {*[t.ɢ][ə]mʔ-s}] (0658a) | `qym` | `head_graph_oc_bs` |
| `38-12` | 闖→tym [*tʰɯms] (None) | `tym` | `head_graph_oc_shengfu` |
| `38-13` | 審→tym [*s.tʰəmʔ {*s.tʰ[ə]mʔ}] (0665a) | `tym` | `head_graph_oc_bs` |
| `38-14` | 冘→lum [*k‧lum] (None); 冘→lu [*ɡ‧lus] (None) | `lum` | `head_graph_mc_coda` |
| `38-15` | 㸒→rym [*N.rəm {*[N.]r[ə]m}] (0657a) | `rym` | `head_graph_oc_bs` |
| `38-16` | 覃→rom [*N.rˤom {*N.rˤ[o]m}] (0646a); 覃→ram [*N-r[a]mʔ] (0646a) | `lim` | `packet_bs_majority` |
| `38-17` | 尋→sym [*sə-ləm {*sə-l[ə]m} (< *-um ?)] (0662a) | `sym` | `head_graph_oc_bs` |
| `38-18` | 林→rym [*k.rəm {*k.r[ə]m}] (0655a) | `rym` | `head_graph_oc_bs` |
| `38-19` | 稟→rim [*p.rimʔ] (0668a); 稟→  *pim [*p.rimʔ (dial. > *primʔ)] (0668a) | `  *pim` | `head_graph_supported_root` |
| `38-20` | 品→pum [*pʰkʳumʔ] (None) | `pum` | `head_graph_oc_shengfu` |
| `38-21` | 貪→rym [*r̥ˤəm {*r̥ˤ[ə]m}] (0645a) | `rym` | `head_graph_oc_bs` |
| `38-22` | 男→nym [*nˤəm {*nˤ[ə]m}] (0649a) | `nym` | `head_graph_oc_bs` |
| `38-23` | 南→nym [*nˤəm {*nˤ[ə]m}] (0650a) | `nym` | `head_graph_oc_bs` |
| `38-24` | 念→nim [*nˤim-s] (0670a) | `nim` | `head_graph_oc_bs` |
| `38-25` | 壬→nym [*nəm {*n[ə]m}] (0667a) | `nym` | `head_graph_oc_bs` |
| `38-26` | 穼→lym [*l̥əm {*[l̥][ə]m}] (None); 深→qum [*qʰlum] (None); 琛→kum [*kʰlum] (None) | `lym` | `head_graph_supported_root` |
| `38-27` | 𠎶→sim [*skʰim] (None) | `sim` | `same_character_series_root` |
| `38-28` | 兓→sum [*skum] (None) | `sum` | `head_graph_oc_shengfu` |
| `38-29` | 參→sum [*srum {*sr[u]m}] (0647a); 參→tsum [*tsʰˤum {*[tsʰ]ˤ[u]m}] (0647a); 參→lum [*s-l̥ˤum {*s-l̥ˤ[u]m}] (0647a) | `sum` | `head_graph_supported_root` |
| `38-30` | 三→sum [*srum {*sr[u]m} (*sr- > *s-; infl. by 四 *s-?)] (0648a); 三→rym [*s.rəm-s {*[s.r][ə]m-s} (*s.r- > *s-, infl. by 四 *s-)] (0648a) | `sum` | `head_graph_mc_onset` |
| `38-31` | 心→sym [*səm] (0663a) | `sym` | `head_graph_oc_bs` |
| `38-32` | 森→sum [*skʳum] (None) | `sum` | `head_graph_oc_shengfu` |
