...
class ResolveSeriesRootsTests(unittest.TestCase):
    ...
    def test_29_27_head_supplement_supplies_zhi_root(self) -> None:
        supplement = resolve_series_roots.load_head_supplement(
            ROOT / "data/series_root_head_supplement.json"
        )
        entry = json.loads((ROOT / "data/entries/curation/29-27.json").read_text(encoding="utf-8"))
        entry = resolve_series_roots.apply_root_resolution(entry, head_supplement=supplement)

        resolved = entry.get("resolved_series_root") or {}
        self.assertEqual(resolved.get("character"), "臸")
        self.assertEqual(resolved.get("root"), "tit")
        self.assertEqual(resolved.get("source"), "head_graph_supplement")

    def test_29_44_head_supplement_supplies_gong_root(self) -> None:
        supplement = resolve_series_roots.load_head_supplement(
            ROOT / "data/series_root_head_supplement.json"
        )
        entry = json.loads((ROOT / "data/entries/curation/29-44.json").read_text(encoding="utf-8"))
        entry = resolve_series_roots.apply_root_resolution(entry, head_supplement=supplement)

        resolved = entry.get("resolved_series_root") or {}
        self.assertEqual(resolved.get("character"), "弜")
        self.assertEqual(resolved.get("root"), "pit")
        self.assertEqual(resolved.get("source"), "head_graph_supplement")

    def test_32_21_head_supplement_supplies_zhen_root(self) -> None:
        supplement = resolve_series_roots.load_head_supplement(
            ROOT / "data/series_root_head_supplement.json"
        )
        entry = json.loads((ROOT / "data/entries/curation/32-21.json").read_text(encoding="utf-8"))
        entry = resolve_series_roots.apply_root_resolution(entry, head_supplement=supplement)

        resolved = entry.get("resolved_series_root") or {}
        self.assertEqual(resolved.get("character"), "絼")
        self.assertEqual(resolved.get("root"), "lin")
        self.assertEqual(resolved.get("source"), "head_graph_supplement")

    def test_33_04_head_supplement_supplies_non_bmp_root(self) -> None:
        supplement = resolve_series_roots.load_head_supplement(
            ROOT / "data/series_root_head_supplement.json"
        )
        entry = json.loads((ROOT / "data/entries/curation/33-04.json").read_text(encoding="utf-8"))
        entry = resolve_series_roots.apply_root_resolution(entry, head_supplement=supplement)

        resolved = entry.get("resolved_series_root") or {}
        self.assertEqual(resolved.get("character"), "𢀷")
        self.assertEqual(resolved.get("root"), "kyn")
        self.assertEqual(resolved.get("source"), "head_graph_supplement")

    def test_35_04_head_supplement_supplies_xie_family_root(self) -> None:
        supplement = resolve_series_roots.load_head_supplement(
            ROOT / "data/series_root_head_supplement.json"
        )
        entry = json.loads((ROOT / "data/entries/curation/35-04.json").read_text(encoding="utf-8"))
        entry = resolve_series_roots.apply_root_resolution(entry, head_supplement=supplement)

        resolved = entry.get("resolved_series_root") or {}
        self.assertEqual(resolved.get("character"), "劦")
        self.assertEqual(resolved.get("root"), "ɡyp")
        self.assertEqual(resolved.get("source"), "head_graph_supported_root")
        self.assertIn("head_graph_supplement", resolved.get("supporting_sources") or [])

    def test_36_13_head_supplement_supplies_yan_variant_root(self) -> None:
        supplement = resolve_series_roots.load_head_supplement(
            ROOT / "data/series_root_head_supplement.json"
        )
        entry = json.loads((ROOT / "data/entries/curation/36-13.json").read_text(encoding="utf-8"))
        entry = resolve_series_roots.apply_root_resolution(entry, head_supplement=supplement)

        resolved = entry.get("resolved_series_root") or {}
        self.assertEqual(resolved.get("character"), "豓")
        self.assertEqual(resolved.get("root"), "lim")
        self.assertEqual(resolved.get("source"), "head_graph_supplement")
